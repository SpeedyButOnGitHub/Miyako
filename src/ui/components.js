...
function toggleModeBtn(id, mode, nextModeLabel, isVC) {
	return btn(
		id,
		nextModeLabel,
		isVC ? ButtonStyle.Success : ButtonStyle.Secondary,
		isVC ? theme.emojis.vc : theme.emojis.text,
	);
}
...
function paginationRow(prefix, page, totalPages) {
	...
	row.addComponents(
		...
		new ButtonBuilder()
			...
			.setLabel(paginationLabel(page, totalPages))
			.setStyle(ButtonStyle.Secondary)
			.setDisabled(true),
		new ButtonBuilder()
...
async function diffEditMessage(target, { embeds, components, content }) {
	try {
		...
		const sameEmbeds =
			JSON.stringify(
				(embeds || []).map((e) => ({ t: e.data?.title, d: e.data?.description, c: e.data?.color })),
			) ===
			JSON.stringify(
				current.map((e) => ({ t: e.data?.title, d: e.data?.description, c: e.data?.color })),
			);
		const norm = (rows) =>
			(rows || []).map((r) =>
				(r.components || [])
