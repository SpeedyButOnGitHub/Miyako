...
function pruneClockInForEvent(evId, options = { clearConsumedAutoNext: true }) {
  try {
    ...
    const clock = ev.__clockIn && typeof ev.__clockIn === 'object' ? { ...ev.__clockIn } : { positions: {}, autoNext: {} };
    const autoNext = clock.autoNext && typeof clock.autoNext === 'object' ? { ...clock.autoNext } : {};
