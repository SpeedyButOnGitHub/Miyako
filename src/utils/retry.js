...
async function retry(fn, opts = {}) {
	const attempts = Number.isFinite(opts.attempts)
		? Math.max(1, opts.attempts)
		: opts.attemptsDefault || 3;
	...
	for (let i = 0; i < attempts; i++) {
		try {
			...
			try {
				logger.warn('[retry] attempt failed', { attempt: i + 1, err: err?.message });
			} catch {}
	...
	try {
		logger.error('[retry] all attempts failed', { attempts, err: lastErr?.message });
	} catch {}
	throw lastErr;
}
