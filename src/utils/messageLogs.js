...
async function logMessageDelete(client, message) {
	...
	const channel = await client.channels.fetch(logChannelId).catch((e) => {
		logError('messageLogs:deleteFetch', e);
		return null;
	});
	...
	try {
		...
		logError('messageLogs:deleteSend', e);
	}
}
...
async function logMessageEdit(client, oldMessage, newMessage) {
	const logChannelId = config.testingMode ? TEST_LOG_CHANNEL : MESSAGE_LOG_CHANNEL;
	const channel = await client.channels.fetch(logChannelId).catch((e) => {
		logError('messageLogs:editFetch', e);
		return null;
	});
	...
	const embed = createEmbed({
		...
		name: (newMessage.author && newMessage.author.tag) || 'Unknown',
		iconURL: newMessage.author ? newMessage.author.displayAvatarURL({ dynamic: true }) : undefined,
	});
	...
	try {
		...
		logError('messageLogs:editSend', e);
	}
}
