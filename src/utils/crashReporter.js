...
function appendEmergency(entry) {
	...
	try {
		fs.appendFileSync(emergencyFile, JSON.stringify(entry) + '\n');
	} catch {
		/* ignore */
	}
}
...
async function gracefulShutdown(reason, err, graceful = false) {
	...
	try {
		...
		mem = { rss: u.rss, heapTotal: u.heapTotal, heapUsed: u.heapUsed };
	} catch {}
	try {
	...
	try {
		logError(graceful ? 'exit' : 'fatal', err || reason);
	} catch {
		appendEmergency(entry);
	}
	...
	try {
		...
		try {
			require('./writeQueue').flushDeferredBackups();
		} catch {}
		...
		try {
			logError('fatal:shutdown', e);
		} catch {
			appendEmergency({ ts: Date.now(), scope: 'fatal:shutdown', message: String(e) });
		}
	}
...
function initEarly() {
	...
	if (!fs.existsSync(CRASH_LATEST_FILE)) {
		safeWrite(
			CRASH_LATEST_FILE,
			JSON.stringify({ ts: Date.now(), status: 'init', note: 'process started' }, null, 2),
		);
	}
	...
	process.on('exit', (code) => {
		if (!fatalHandled) {
			if (code !== 0) {
				...
				try {
					logError('fatal:exit', `abnormal exit code ${code}`);
				} catch {}
				safeWrite(
					CRASH_LATEST_FILE,
					JSON.stringify(
						{ ts: Date.now(), scope: 'fatal', reason: 'abnormal-exit', code },
						null,
						2,
					),
				);
				...
				try {
					logError('exit', `process exiting with code ${code}`);
				} catch {}
			}
		}
	});
...
function attachClient(client) {
	...
	try {
		const latest = fs.existsSync(CRASH_LATEST_FILE)
			? JSON.parse(fs.readFileSync(CRASH_LATEST_FILE, 'utf8'))
			: null;
