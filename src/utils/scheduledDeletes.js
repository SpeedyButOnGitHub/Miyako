...
function ensure() {
	try {
		...
		if (!fs.existsSync(FILE)) fs.writeFileSync(FILE, JSON.stringify({ messages: {} }, null, 2));
	} catch {}
}
function load() {
	ensure();
...
function setForMessage(messageId, entry) {
	const db = load();
	db.messages = db.messages || {};
	db.messages[messageId] = entry;
	save(db);
	return db.messages[messageId];
}
function removeForMessage(messageId) {
	const db = load();
	db.messages = db.messages || {};
	if (messageId in db.messages) delete db.messages[messageId];
	save(db);
}
