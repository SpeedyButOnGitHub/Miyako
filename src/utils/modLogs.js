...
async function sendModLog(
	...
	if (isAutoKick) bottomParts.push(`🚨 This user has been kicked due to reaching ${currentWarnings} warning${currentWarnings === 1 ? "" : "s"}.`);
	...
	const embed = createEmbed({
		...
		timestamp: false
	}).setAuthor({ name: modTag, iconURL: (typeof modUser?.displayAvatarURL === 'function') ? modUser.displayAvatarURL({ dynamic: true }) : undefined });
	...
	const avatarUrl = typeof targetUser?.displayAvatarURL === 'function' ? targetUser.displayAvatarURL({ dynamic: true, size: 256 }) : null;
