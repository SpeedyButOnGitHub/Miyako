...
function saveSettingMeta() {
	try {
		fs.writeFileSync(SETTING_META_FILE, JSON.stringify(settingMeta, null, 2));
	} catch {}
}
...
function validateConfig(cfg) {
	...
	if (typeof cfg.escalation !== 'object' || cfg.escalation === null)
		cfg.escalation = { ...defaultConfig.escalation };
	if (typeof cfg.defaultMuteDuration !== 'number')
		cfg.defaultMuteDuration = defaultConfig.defaultMuteDuration;
	...
	if (typeof cfg.vcLevelRewards !== 'object' || cfg.vcLevelRewards === null)
		cfg.vcLevelRewards = {};
	if (['whitelist', 'blacklist'].includes(cfg.levelingMode) === false)
		cfg.levelingMode = 'blacklist';
	if (!Array.isArray(cfg.levelingChannelList)) cfg.levelingChannelList = [];
	if (!Array.isArray(cfg.roleXPBlacklist)) cfg.roleXPBlacklist = [];
	if (typeof cfg.globalXPMultiplier !== 'number' || !Number.isFinite(cfg.globalXPMultiplier))
		cfg.globalXPMultiplier = 1.0;
	...
	if (cfg.moderatorLogChannelId && typeof cfg.moderatorLogChannelId !== 'string')
		cfg.moderatorLogChannelId = String(cfg.moderatorLogChannelId);

	// Command logging defaults
	if (typeof cfg.commandLogging !== 'object' || cfg.commandLogging === null)
		cfg.commandLogging = {};
	...
	if (typeof cfg.commandLogging.testingCompare !== 'boolean')
		cfg.commandLogging.testingCompare = true;
	...
	if (typeof cfg.escalation.muteThreshold !== 'number')
		cfg.escalation.muteThreshold = defaultConfig.escalation.muteThreshold;
	if (typeof cfg.escalation.muteDuration !== 'number')
		cfg.escalation.muteDuration = defaultConfig.escalation.muteDuration;
	if (typeof cfg.escalation.kickThreshold !== 'number')
		cfg.escalation.kickThreshold = defaultConfig.escalation.kickThreshold;

	// Validate economy.cashDrops
	if (typeof cfg.cashDrops !== 'object' || cfg.cashDrops === null)
		cfg.cashDrops = { ...defaultConfig.cashDrops };
	const e = cfg.cashDrops;
	const clamp = (x, lo, hi) => Math.max(lo, Math.min(hi, x));
	e.dropChance = Number.isFinite(e.dropChance)
		? clamp(e.dropChance, 0, 1)
		: defaultConfig.cashDrops.dropChance;
	e.minAmount = Number.isFinite(e.minAmount)
		? Math.max(0, Math.floor(e.minAmount))
		: defaultConfig.cashDrops.minAmount;
	e.maxAmount = Number.isFinite(e.maxAmount)
		? Math.max(e.minAmount, Math.floor(e.maxAmount))
		: Math.max(e.minAmount, defaultConfig.cashDrops.maxAmount);
	e.lifetimeMs = Number.isFinite(e.lifetimeMs)
		? Math.max(5 * 1000, Math.floor(e.lifetimeMs))
		: defaultConfig.cashDrops.lifetimeMs;
...
try {
	...
	try {
		require('./logger').warn('[Config] read/parse failed; rewriting defaults', {
...
function saveConfig() {
	try {
		...
		try {
			require('./logger').warn('[Config] save failed', { err: err?.message || String(err) });
		} catch {}
	}
}
