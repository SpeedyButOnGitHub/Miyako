...
function ensure() {
  try { if (!fs.existsSync(PRIV_DIR)) fs.mkdirSync(PRIV_DIR, { recursive: true }); } catch {}
  try { if (!fs.existsSync(FILE)) fs.writeFileSync(FILE, JSON.stringify({ events: {} }, null, 2)); } catch {}
}
