	...
	if (Object.keys(runtimePatch).length) {
		try {
			...
			if (runtimePatch.__clockIn && typeof runtimePatch.__clockIn === 'object') {
				const curClock = (curRt.__clockIn && typeof curRt.__clockIn === 'object') ? { ...curRt.__clockIn } : {};
				...
				const curPositions = (curClock.positions && typeof curClock.positions === 'object') ? { ...curClock.positions } : {};
				const patchPositions = (patchClock.positions && typeof patchClock.positions === 'object') ? { ...patchClock.positions } : null;
				if (patchPositions) {
			...
			if (runtimePatch.__notifMsgs && typeof runtimePatch.__notifMsgs === 'object') {
				const curNot = (curRt.__notifMsgs && typeof curRt.__notifMsgs === 'object') ? { ...curRt.__notifMsgs } : {};
				const patchNot = { ...runtimePatch.__notifMsgs };
				// shallow merge per notification id
				const mergedNot = { ...curNot };
				for (const nid of Object.keys(patchNot)) mergedNot[nid] = { ...mergedNot[nid], ...patchNot[nid] };
