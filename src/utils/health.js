...
async function runHealthChecks(client) {
			...
			if (!ev.anchorChannelId || !ev.anchorMessageId) {
				...
				try {
					...
					results.push({
						...
						name: ev.name,
						ok: false,
						error: 'anchor create failed: ' + e.message,
					});
					continue;
				}
			}
				...
				try {
					...
					if (/Midnight Bar/i.test(ev.name || '')) {
						...
						const hasButton =
							...
							msg.components.some((r) => r.components?.some?.((c) => c.customId === expectedId));
						if (!hasButton) {
							buttonHealthy = false;
							try {
								...
								if (repaired) {
									const repairedHas =
										Array.isArray(repaired.components) &&
										repaired.components.some((r) =>
					...
					try {
						if (
							...
							ev.ranges &&
							Array.isArray(ev.ranges) &&
							ev.ranges.length
						) {
							const baseContent =
								ev.dynamicBaseContent || ev.messageJSON?.content || ev.message || msg.content || '';
							...
							const sh = parseInt(shRaw, 10) || 0;
							const sm = parseInt(smRaw, 10) || 0;
							const now = new Date();
							...
							if (lineRegex.test(baseContent)) {
								...
								if (newContent !== msg.content) {
									...
									if (newContent !== ev.dynamicBaseContent)
										updateEvent(ev.id, { dynamicBaseContent: newContent });
								}
							}
						}
					} catch (tsErr) {
						/* ignore timestamp fix errors */
					}
					results.push({
						kind: 'event',
						id: ev.id,
						...
						url: buttonHealthy
							...
							: null,
						error: buttonHealthy ? undefined : 'notification button missing',
					});
				} catch (e) {
	...
	try {
		...
		if (staffMsg) {
			results.push({
				...
				name: 'Staff Team',
				ok: true,
				url: `https://discord.com/channels/${staffMsg.guildId}/${staffMsg.channelId}/${staffMsg.id}`,
			});
		} else {
			results.push({
				kind: 'staffTeam',
	...
	try {
		...
		results.push({
			...
			id: 'activeMenus',
			name: 'Active Menus',
			ok: stale === 0 && emptyRows === 0,
			error: stale || emptyRows ? `stale:${stale} emptyRows:${emptyRows}` : undefined,
		});
	} catch (e) {
		results.push({
			kind: 'activeMenus',
