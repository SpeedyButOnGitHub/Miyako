...
async function runStartupTests(client) {
  ...
  try {
    ...
    let prev = null;
    try { if (fs.existsSync(snap)) prev = JSON.parse(fs.readFileSync(snap, 'utf8')); } catch {}
    try {
      const curr = createSnapshot(projectRoot);
      const diff = compareSnapshots(prev, curr);
      add('changelog_snapshot', true, { added: diff.added.length, removed: diff.removed.length, modified: diff.modified.length });
    } catch (e) {
  ...
  try {
    ...
    if (client) {
      const health = await runHealthChecks(client).catch((e) => { throw e; });
