...
try {
	require('../scripts/migrate-runtime-data').migrate();
} catch {}
// Prevent multiple concurrent bot instances (esp. if accidentally started twice)
try {
	require('./utils/singleton').ensureSingleton();
} catch {}
try {
	process.title = 'MiyakoBot';
} catch {}
...
try {
	...
	if (fs.existsSync(lastShutdownPath)) {
		...
		if (raw && raw.ts) {
			...
			try {
				fs.unlinkSync(lastShutdownPath);
			} catch {}
		}
	}
} catch {}
...
try {
	delete require.cache[require.resolve('../events/messageEvents')];
} catch {}
...
try {
	require('./utils/depositProgress').load();
} catch {}
...
registerErrorListener(async (entry) => {
	try {
		...
		if (!CRASH_LOG_CHANNEL_ID)
			CRASH_LOG_CHANNEL_ID = config.crashLogChannelId || CONFIG_LOG_CHANNEL;
	...
	console.error = function (...args) {
		try {
			recordExternalError(
				...
				args.length === 1
					? args[0]
					: args.map((a) => (a && a.stack ? a.stack : String(a))).join(' '),
			);
		} catch {
			/* ignore */
		}
...
try {
	require('./utils/crashReporter').attachClient(client);
} catch {}
...
async function sendBotStatusMessage() {
	...
	if (fs.existsSync(BOT_STATUS_FILE)) {
		try {
			lastOnline = JSON.parse(fs.readFileSync(BOT_STATUS_FILE)).lastOnline || 0;
		} catch {}
	}
	const now = Date.now();
	...
	if (channel) {
			...
			try {
				...
				if (total === 0) {
					safeAddField(
						embed,
						'Changelog Overview',
						'No changes have been made since last restart.',
					);
				} else {
		...
		try {
			...
			try {
				if (CONFIG_LOG_CHANNEL) {
					...
					if (ch) {
						const sEm = createEmbed({
							title: 'Startup Tests',
							description: summary.ok
								? 'All quick checks passed ✅'
								: 'Some quick checks failed ⚠️',
							color: summary.ok ? 0x2ecc71 : 0xe67e22,
						});
						...
						if (summary.checks) {
							...
							if (cs && cs.ok && cs.info) {
								sEm.addFields({
									name: 'Changelog',
									value: `➕ ${cs.info.added}  ✖️ ${cs.info.removed}  🔧 ${cs.info.modified}`,
								});
							}
						}
						...
						if (summary.health && Array.isArray(summary.health)) {
							...
							if (nonOk.length) {
								const lines = nonOk
									.map((n) => `• ${n.name || n.kind}: ${n.error || 'unknown'}`)
									.join('\n');
				...
				logger.warn('[StartupTests] could not send summary', { err: e && e.message });
			}
		} catch (e) {
			logger.warn('[StartupTests] runner failed', { err: e && e.message });
		}
		...
		if (changelogSession && changelogSession.detailLines && changelogSession.detailLines.length) {
			components = [
				buildNavRow([semanticButton('primary', { id: 'status_show', label: 'Details' })]),
			];
		}
		...
		if (sent && changelogSession) {
			try {
				ActiveMenus.registerMessage(sent, {
					type: 'status',
					data: { ...changelogSession, expanded: false },
				});
			} catch {}
		}
	}
...
async function setStatusChannelName(online) {
	...
	const name = online ? '🟢︎𝙼𝚒𝚢𝚊𝚔𝚘-𝚜𝚝𝚊𝚝𝚞𝚜' : '🔴︎𝙼𝚒𝚢𝚊𝚔𝚘-𝚜𝚝𝚊𝚝𝚞𝚜';
...
client.once('clientReady', async () => {
		...
		if (issues.length) {
			...
			if (CONFIG_LOG_CHANNEL) {
				...
				if (channel) {
					...
					let prev = null;
					try {
						if (fs.existsSync(stampFile)) prev = fs.readFileSync(stampFile, 'utf8').trim();
					} catch {}
					if (prev !== hash) {
						channel
							.send({
	...
	try {
		await ActiveMenus.init(client);
	} catch (e) {
		logger.error('[ActiveMenus init]', { err: e.message });
	}
	// Sweep orphaned (stale) interactive menus (older than 1h) for cleanliness
	try {
		if (ActiveMenus.sweepOrphans) await ActiveMenus.sweepOrphans(client);
	} catch {}
	...
	try {
		...
		logger.error('[Scheduler] start error', { err: e.message });
	}
	...
	try {
		...
		try {
			const sdPath = require('path').join(
				require('./utils/paths').dataDir(),
				'private',
				'scheduledDeletes.json',
			);
			try {
				if (require('fs').existsSync(sdPath)) require('fs').unlinkSync(sdPath);
			} catch {}
		} catch {}
		try {
			await reconstructScheduledDeletes(client);
		} catch (e) {
			logger.warn('[reconstructScheduledDeletes] failed', { err: e && e.message });
		}
	} catch (e) {
		logger.warn('[reconstructScheduledDeletes] require failed', { err: e && e.message });
	}
	...
	try {
		...
		logger.error('[VoiceLeveling] start error', { err: e.message });
	}
	...
	try {
		...
		logger.error('[CashDrops] start error', { err: e.message });
	}
	...
	try {
		...
		for (const ev of evs) {
			...
			// surviving a restart)
			try {
				await refreshTrackedAutoMessages(client, ev, { forceAll: true });
			} catch {}
			await new Promise((r) => setTimeout(r, 150));
		}
	} catch (e) {
		logger.error('[Startup Refresh] error', { err: e.message });
	}
	...
	if (fs.existsSync(ACTIVE_MENUS_FILE)) {
		try {
			...
			try {
				...
				const isOpen = (ev) => {
					try {
						...
						if (Array.isArray(ev.times) && ev.times.includes(currentHM)) {
							if (Array.isArray(ev.days) && ev.days.length && !ev.days.includes(currentDay))
								return false;
							return true;
						}
					} catch {}
				...
				logger && logger.warn && logger.warn('[Startup Reconcile] failed', { err: e && e.message });
			}
			fs.writeFileSync(ACTIVE_MENUS_FILE, '[]');
		} catch (err) {
			logger.warn('[Startup Menu Cleanup Error]', { err: err.message });
		}
	}
});
	...
	ActiveMenus.registerHandler('status', async (interaction, session) => {
		...
		if (interaction.customId === 'status_show') {
			...
			if (data.detailLines && data.detailLines.length) {
				...
				if (chunked[1])
					filtered.push({ name: 'Details (cont.)', value: chunked[1].slice(0, 1024) });
			}
			embed.setFields(filtered);
			const rows = [buildNavRow([semanticButton('nav', { id: 'status_hide', label: 'Hide' })])];
		...
		if (interaction.customId === 'status_hide') {
			...
			embed.setFields(
				fields.map((f) => (f.name === 'Changelog' ? { ...f, name: 'Changelog Overview' } : f)),
			);
			const rows = [
				buildNavRow([semanticButton('primary', { id: 'status_show', label: 'Details' })]),
			];
