	...
	try {
		...
		try {
			if (app.messageJSON) {
				...
				if (Array.isArray(app.messageJSON.components) && app.messageJSON.components.length)
					payload.components = app.messageJSON.components;
			...
			if (!payload) {
				const emb = createEmbed({
					title: app.name || `Application #${app.id}`,
				...
				if (app.submissionChannelId)
					safeAddField(
						emb,
						'Submission Channel',
						app.submissionChannelId ? `<#${app.submissionChannelId}>` : '*none*',
					);
				payload = { embeds: [emb] };
			}
		} catch (e) {
		...
		try {
			...
			const tmpForSanitize = {
				embed: Array.isArray(payload.embeds) ? payload.embeds[0] : null,
				components: payload.components,
				content: payload.content,
			};
			const sanitized = sanitizeMessageJSON(tmpForSanitize);
			const finalPayload = {};
			if (sanitized.embed) finalPayload.embeds = [sanitized.embed];
			else if (payload.embeds) finalPayload.embeds = payload.embeds;
			...
			try {
				require('../utils/logger').warn('[applications] edit deployed message failed', {
					err: e && e.message,
				});
			} catch {}
		...
		try {
			require('../utils/logger').error('[applications] tryEditDeployedApplication error', {
				err: e && e.message,
			});
		} catch {}
		return 'failed';
...
function validateMessageJSON(obj) {
	...
	if (emb !== undefined) {
		if (!emb || typeof emb !== 'object') errors.push('embed must be an object');
			...
			if (typeof emb.title === 'string' && emb.title.length > 256)
				errors.push('embed.title too long (max 256)');
			if (typeof emb.description === 'string' && emb.description.length > 4096)
				errors.push('embed.description too long (max 4096)');
			if (Array.isArray(emb.fields) && emb.fields.length > 25)
				errors.push('embed.fields too many (max 25)');
		}
	}
		...
		if (!Array.isArray(comps)) errors.push('components must be an array');
			...
			for (const row of comps) {
				if (!row || typeof row !== 'object') {
					errors.push('component row must be an object');
					continue;
				}
				const children = row.components || row['components'];
				if (!Array.isArray(children)) {
					errors.push('component row must have components array');
					continue;
				}
				if (children.length > 5) errors.push('too many components in a row (max 5)');
				for (const child of children) {
					if (!child || typeof child !== 'object') {
						errors.push('component must be object');
						continue;
					}
					if (child.type === 2 || child.style) {
						// button-like
						if (child.custom_id && String(child.custom_id).length > 100)
							errors.push('button custom_id too long (max 100)');
						if (child.label && String(child.label).length > 80)
							errors.push('button label too long (max 80)');
					}
				}
			}
		}
				...
				safeRow.components = children
					.map((child) => {
						...
						if (Array.isArray(outChild.options)) {
							outChild.options = outChild.options
								.map((opt) => {
...
function buildRootEmbed() {
	...
	safeAddField(
		...
		apps.length
			? apps
					.slice(0, 10)
					.map((a) => `${a.enabled ? '🟢' : '🔴'} #${a.id} **${a.name}** (${a.questions.length} q)`)
					.join('\n')
			: '*(none)*',
	);
	safeAddField(
		...
		panels.length
			? panels
					.slice(0, 10)
					.map((p) => `#${p.id} **${p.name}** (${p.applicationIds.length} apps)`)
					.join('\n')
			: '*(none)*',
	);
	return e;
...
function buildApplicationsListEmbed(page = 0, pageSize = 5) {
	...
	const e = createEmbed({
		title: 'Applications',
		description: `Page ${page + 1}/${totalPages} • ${apps.length} total`,
	});
	if (!apps.length) {
...
function buildApplicationsListComponents(page, totalPages, apps) {
	...
	const createBtn = semanticButton('success', {
		id: 'appmgr_apps_create',
		label: 'Create',
		emoji: theme.emojis.create,
	});
	const deleteBtn = semanticButton('danger', {
		id: 'appmgr_apps_delete',
		label: 'Delete',
		emoji: theme.emojis.delete,
	});
	...
	try {
		...
		if (!allApps.length) {
			const selectionRow = buildNavRow([
				semanticButton('secondary', { id: 'noop', label: 'No applications', enabled: false }),
			]);
		...
		const appButtons = allApps.map((a) =>
			semanticButton('primary', { id: `appmgr_app_select_${a.id}`, label: a.name.slice(0, 20) }),
		);
		const selectionRows = appButtons.length
			? splitButtonsIntoRows(appButtons)
			: [
					buildNavRow([
						semanticButton('secondary', { id: 'noop', label: 'No applications', enabled: false }),
					]),
...
function buildPanelsListEmbed(page = 0, pageSize = 5) {
	...
	const e = createEmbed({
		title: 'Panels',
		description: `Page ${page + 1}/${totalPages} • ${panels.length} total`,
	});
...
function buildPanelsListComponents(page, totalPages, panels) {
	...
	const prev = semanticButton('nav', {
		id: 'appmgr_panels_prev',
		label: 'Prev',
		enabled: page > 0,
	});
	const next = semanticButton('nav', {
		id: 'appmgr_panels_next',
		label: 'Next',
		enabled: page < totalPages - 1,
	});
	const createBtn = semanticButton('success', {
		id: 'appmgr_panels_create',
		label: 'Create',
		emoji: theme.emojis.create,
	});
	const deleteBtn = semanticButton('danger', {
		id: 'appmgr_panels_delete',
		label: 'Delete',
		emoji: theme.emojis.delete,
	});
	const back = backButton('appmgr_back_root', 'Back');
	const panelButtons = allPanels.map((p) =>
		semanticButton('primary', { id: `appmgr_panel_select_${p.id}`, label: p.name.slice(0, 20) }),
	);
	const selectionRows = panelButtons.length
		? splitButtonsIntoRows(panelButtons)
		: [
				buildNavRow([
					semanticButton('secondary', { id: 'noop', label: 'No panels', enabled: false }),
				]),
			];
	const bottomItems = [];
...
function buildAppDetailEmbed(appId) {
	const app = getApplication(appId);
	if (!app)
		return createEmbed({ title: 'Missing Application', description: 'It may have been removed.' });
	...
	if (app.requiredRoles?.length)
		safeAddField(e, 'Required Roles', app.requiredRoles.map((r) => `<@&${r}>`).join(' '));
	if (app.managerRoles?.length)
		safeAddField(e, 'Manager Roles', app.managerRoles.map((r) => `<@&${r}>`).join(' '));
	if (app.acceptedRoles?.length)
		safeAddField(e, 'Accepted Roles', app.acceptedRoles.map((r) => `<@&${r}>`).join(' '));
	if (app.pendingRole) safeAddField(e, 'Pending Role', `<@&${app.pendingRole}>`, true);
	if (app.restrictedRoles?.length)
		safeAddField(e, 'Restricted Roles', app.restrictedRoles.map((r) => `<@&${r}>`).join(' '));
	if (app.deniedRoles?.length)
		safeAddField(e, 'Denied Roles', app.deniedRoles.map((r) => `<@&${r}>`).join(' '));
	return e;
}
...
function buildAppDetailComponents(appId, expanded = false) {
	...
	if (!expanded && actions.length > visibleCount) {
		...
		try {
			...
			const sel = new StringSelectMenuBuilder()
				.setCustomId(`appmgr_app_menu_${app.id}`)
				.setPlaceholder('More…')
				.setMinValues(1)
				.setMaxValues(1);
			...
			const selRow = new ARB();
			selRow.addComponents(sel);
	...
	if (expanded) {
		...
		if (rem.length) {
			...
			for (const a of rem) {
				const lbl = compact(a.label, 14);
				const b = semanticButton(a.kind === 'primary' ? 'primary' : 'nav', {
					id: a.id,
					label: lbl,
					emoji: a.emoji,
				});
				if (r2.components.length >= 5) {
					rows.push(r2);
					r2 = buildNavRow([]);
				}
	...
	const toggle = semanticButton('toggle', {
		id: `appmgr_app_toggle_${app.id}`,
		label: app.enabled ? 'Disable' : 'Enable',
		active: app.enabled,
	});
	const del = semanticButton('danger', {
		id: `appmgr_app_delete_${app.id}`,
		label: 'Delete',
		emoji: theme.emojis.delete,
	});
	const bottomRow = buildNavRow([
		toggle,
		back,
		del,
		...(expanded
			? [semanticButton('nav', { id: `appmgr_app_less_${app.id}`, label: 'Less' })]
			: []),
	]);
...
function buildQuestionListEmbed(appId, page = 0) {
	...
	slice.forEach((q, i) => {
		safeAddField(
			e,
			`${page * perPage + i + 1}. ${q.label.slice(0, 230)} ${q.required ? '(required)' : ''}`,
			`ID: ${q.id} • Type: ${q.type}`,
		);
	});
...
function buildPanelDetailEmbed(panelId) {
	...
	if (!panel)
		return createEmbed({ title: 'Missing Panel', description: 'It may have been removed.' });
	const e = createEmbed({ title: `Panel #${panel.id}`, description: panel.name });
	safeAddField(
		e,
		'Applications',
		panel.applicationIds.length ? panel.applicationIds.map((id) => `#${id}`).join(', ') : '*none*',
	);
...
function buildPanelDetailComponents(panelId) {
	...
	const rename = semanticButton('primary', {
		id: `appmgr_panel_rename_${panel.id}`,
		label: 'Rename',
		emoji: theme.emojis.edit,
	});
	const desc = semanticButton('primary', { id: `appmgr_panel_desc_${panel.id}`, label: 'Desc' });
	const chan = semanticButton('primary', {
		id: `appmgr_panel_channel_${panel.id}`,
		label: 'Channel',
	});
	const appsManage = semanticButton('primary', {
		id: `appmgr_panel_apps_${panel.id}`,
		label: 'Apps',
	});
	const deploy = semanticButton('success', {
		id: `appmgr_panel_deploy_${panel.id}`,
		label: 'Deploy',
	});
	const del = semanticButton('danger', {
		id: `appmgr_panel_delete_${panel.id}`,
		label: 'Delete',
		emoji: theme.emojis.delete,
	});
...
async function handleApplicationsCommand(client, message) {
	...
	if (message.author.id !== getOwnerId()) {
		return message.reply({
			content: 'This command is currently restricted.',
	...
	try {
		...
		try {
			require('../utils/logger').error('[applications] send failed', { err: e.message });
		} catch {}
		return message.reply({
			content: 'Failed to open Applications Manager (logged).',
			allowedMentions: { repliedUser: false },
		});
	}
	...
	try {
		...
		if (id === 'appmgr_app_select_menu' && interaction.isStringSelectMenu() && interaction.values) {
			...
			if (!getApplication(appId)) {
				try {
					require('../utils/logger').warn('[applications] select-menu missing app', { appId });
				} catch (e) {}
				return interaction
					.reply({ content: 'Application not found.', flags: 1 << 6 })
					.catch(() => {});
			}
			session.data.view = 'appDetail';
			session.data.appId = appId;
			session.data.editSection = null;
			...
			try {
				...
				if (typeof userPrefs.appExpanded === 'boolean')
					session.data.appExpanded = userPrefs.appExpanded;
			} catch {}
			const row1 = buildNavRow([
				semanticButton('toggle', {
					id: `appmgr_app_toggle_${app.id}`,
					label: app.enabled ? 'Disable' : 'Enable',
					active: app.enabled,
				}),
		...
		if (id === 'appmgr_panels') {
			session.data.view = 'panels';
			session.data.page = 0;
			const { embed, page, totalPages, panels } = buildPanelsListEmbed(0);
			return interaction.update({
				embeds: [embed],
				components: buildPanelsListComponents(page, totalPages, panels),
			});
		}
		if (id === 'appmgr_new_app') {
			const created = addApplication({});
			session.data.view = 'appDetail';
			session.data.appId = created.id;
			return interaction.update({
				embeds: [buildAppDetailEmbed(created.id)],
				components: buildAppDetailComponents(created.id),
			});
		}
		// Create from list
		if (id === 'appmgr_apps_create') {
			...
			try {
				...
				try {
					require('../utils/logger').error('[applications] create app failed', {
						err: e.message,
						stack: e.stack,
					});
				} catch {}
				return interaction
					.reply({ content: 'Failed to create application.', flags: 1 << 6 })
					.catch(() => {});
			}
		}
		if (id === 'appmgr_new_panel') {
			...
			session.data.panelId = created.id;
			return interaction.update({
				embeds: [buildPanelDetailEmbed(created.id)],
				components: buildPanelDetailComponents(created.id),
			});
		}
		if (id === 'appmgr_panels_create') {
			...
			try {
				...
				try {
					require('../utils/logger').error('[applications] create panel failed', {
						err: e.message,
						stack: e.stack,
					});
				} catch {}
				return interaction
					.reply({ content: 'Failed to create panel.', flags: 1 << 6 })
					.catch(() => {});
			}
		}
		// Pagination (apps)
		if (id === 'appmgr_apps_prev' || id === 'appmgr_apps_next') {
			...
			return interaction.update({
				embeds: [embed],
				components: buildApplicationsListComponents(page, tp, pageApps),
			});
		}
		// Pagination (panels)
		if (id === 'appmgr_panels_prev' || id === 'appmgr_panels_next') {
			...
			return interaction.update({
				embeds: [embed],
				components: buildPanelsListComponents(page, tp, pagePanels),
			});
		}
		...
		if (id.startsWith('appmgr_app_select_')) {
			...
			if (!app) {
				try {
					require('../utils/logger').warn('[applications] selected missing app', { appId });
				} catch {}
				session.data.view = 'apps';
				session.data.appId = null;
				const { embed, page, totalPages, apps } = buildApplicationsListEmbed(
					session.data.page || 0,
				);
				return interaction.update({
					embeds: [embed],
			...
			session.data.appId = appId;
			return interaction.update({
				embeds: [buildAppDetailEmbed(appId)],
				components: buildAppDetailComponents(appId),
			});
		}
		...
		if (id.startsWith('appmgr_app_menu_')) {
			...
			const app = getApplication(appId);
			if (!app)
				return interaction.reply({ content: 'App missing.', flags: 1 << 6 }).catch(() => {});
			// Grab selection (single)
			const sel = interaction.values && interaction.values[0];
			if (!sel)
				return interaction.reply({ content: 'No selection.', flags: 1 << 6 }).catch(() => {});
			...
			if (sel.includes('appmgr_app_questions_')) {
				session.data.view = 'questions';
				session.data.qPage = 0;
				session.data.appId = appId;
				const { embed, page, totalPages } = buildQuestionListEmbed(appId, 0);
				return interaction.update({
					embeds: [embed],
					components: buildQuestionListComponents(appId, page, totalPages),
				});
			}
			if (sel.includes('appmgr_app_msgs_')) {
				session.data.view = 'appMsgs';
				session.data.appId = appId;
				session.data.editSection = 'messages';
				const a = getApplication(appId);
				const e = createEmbed({
					title: `📨 Messages • App #${appId}`,
					description: a.name || 'Messages for this application',
				});
				// Compact inline fields with emoji labels
				safeAddField(
					e,
					'✅ Accept',
					a.acceptMessage ? `${a.acceptMessage.slice(0, 120)}` : '*(none)*',
					true,
				);
				safeAddField(
					e,
					'❌ Deny',
					a.denyMessage ? `${a.denyMessage.slice(0, 120)}` : '*(none)*',
					true,
				);
				safeAddField(
					e,
					'✉️ Confirm',
					a.confirmMessage ? `${a.confirmMessage.slice(0, 120)}` : '*(none)*',
					true,
				);
				safeAddField(
					e,
					'🏁 Completion',
					a.completionMessage ? `${a.completionMessage.slice(0, 120)}` : '*(none)*',
					true,
				);
			...
			if (sel.includes('appmgr_app_deployed_')) {
				...
				return interaction.update({
					embeds: [buildAppDetailEmbed(appId)],
					components: buildAppDetailComponents(appId, session.data.appExpanded),
				});
			}
			if (sel.includes('appmgr_app_editmsg_')) {
				...
				if (fullJSON.length > 3800) {
					...
					return interaction.reply({
						content: 'Stored payload is too large to edit in a modal. See attached file.',
						files: [{ attachment: buffer, name: `app_${appId}_messageJSON.json` }],
						flags: 1 << 6,
					});
				}
				const m = new ModalBuilder()
					.setCustomId(modalId)
					.setTitle('Edit Stored Message JSON')
					.addComponents(
						new ActionRowBuilder().addComponents(
							new TextInputBuilder()
								...
								.setRequired(true)
								.setValue(fullJSON.slice(0, 4000)),
						),
					);
			...
			if (sel.includes('appmgr_app_props_')) {
				session.data.view = 'appProps';
				session.data.appId = appId;
				session.data.editSection = 'properties';
				const a = getApplication(appId);
				...
				safeAddField(
					e,
					'Submission Channel',
					a.submissionChannelId ? `<#${a.submissionChannelId}>` : '*none*',
				);
				...
				const row = buildNavRow([
					...
					semanticButton('danger', {
						id: `appprops_clearchan_${appId}`,
						label: 'ClrChan',
						enabled: !!a.submissionChannelId,
					}),
			...
			if (sel.includes('appmgr_app_roles_')) {
				session.data.view = 'appRoles';
				session.data.appId = appId;
				session.data.editSection = 'roles';
				const a = getApplication(appId);
		...
		if (id.startsWith('appmgr_panel_select_')) {
			...
			if (!panel) {
				try {
					require('../utils/logger').warn('[applications] selected missing panel', { panelId });
				} catch {}
				session.data.view = 'panels';
				session.data.panelId = null;
			...
			session.data.view = 'panelDetail';
			session.data.panelId = panelId;
			return interaction.update({
				embeds: [buildPanelDetailEmbed(panelId)],
				components: buildPanelDetailComponents(panelId),
			});
		}
		...
		if (id.startsWith('appmgr_app_toggle_')) {
			...
			if (app) {
				...
				try {
					return interaction.update({ embeds: [refreshed], components: comps });
				} catch (e) {
					try {
						require('../utils/logger').error('[applications] toggle failed', { err: e.message });
					} catch {}
				}
			}
			return interaction
		...
		if (id.startsWith('appmgr_app_trigger_')) {
			...
			if (!ch || !ch.send)
				return interaction
					.reply({ content: 'Target channel unavailable.', flags: 1 << 6 })
					.catch(() => {});
			try {
				const { ActionRowBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
				const emb = createEmbed({
					title: app.name || `Application #${app.id}`,
					description: app.name || 'Apply using the button below.',
				});
				safeAddField(emb, 'Questions', String((app.questions || []).length));
				const row = new ActionRowBuilder().addComponents(
					new ButtonBuilder()
						.setCustomId(`apply_app_${app.id}`)
						.setLabel('Apply')
						.setStyle(ButtonStyle.Primary),
				);
				await ch.send({ embeds: [emb], components: [row] });
				return interaction
					.reply({ content: 'Application triggered.', flags: 1 << 6 })
					.catch(() => {});
			} catch (e) {
				try {
					require('../utils/logger').error('[applications] trigger failed', { err: e.message });
				} catch {}
				return interaction
					.reply({ content: 'Failed to trigger application.', flags: 1 << 6 })
					.catch(() => {});
			}
		}
		...
		if (id.startsWith('appmgr_app_edit_')) {
			...
			session.data.view = 'appDetail';
			session.data.appId = appId;
			session.data.editSection = 'questions';
			...
			return interaction.update({
				...
				components: [
					header,
					...buildQuestionListComponents(
						appId,
						0,
						Math.max(1, Math.ceil((app.questions || []).length / 6)),
					),
				],
			});
		}
		...
		if (id.startsWith('appmgr_app_deployed_')) {
			...
			if (!app)
				return interaction
					.reply({ content: 'Application missing.', flags: 1 << 6 })
					.catch(() => {});
			const { EmbedBuilder } = require('discord.js');
			const e = new EmbedBuilder()
				.setTitle(`Deployed • App #${app.id}`)
				.setColor(0x2f3136)
				.setTimestamp();
			...
			// saved payload size
			try {
				const size = app.messageJSON
					? Buffer.byteLength(JSON.stringify(app.messageJSON), 'utf8')
					: 0;
			...
			if (hasRef) {
				try {
					...
					const msg =
						ch && ch.messages ? await ch.messages.fetch(app.messageId).catch(() => null) : null;
		...
		if (id.startsWith('appmgr_app_editmsg_')) {
			...
			if (fullJSON.length > 3800) {
				...
				try {
					...
					return interaction.reply({
						content:
							'Stored payload is too large to edit in a modal. See attached file. To update, copy the JSON, edit locally, then use the `EditMsg` action again with a smaller payload.',
						files: [{ attachment: buffer, name: `app_${appId}_messageJSON.json` }],
						flags: 1 << 6,
					});
				} catch (e) {
					return interaction
						.reply({
							content:
								'Stored payload is too large and could not be attached. Please retrieve the JSON from the filesystem.',
							flags: 1 << 6,
						})
						.catch(() => {});
				}
			...
			const m = new ModalBuilder()
				...
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('json')
							.setLabel('JSON payload (embed/components/content)')
							.setStyle(TextInputStyle.Paragraph)
							.setRequired(true)
							.setValue(initial),
					),
			...
			try {
				...
				return submitted
					.reply({ content: 'Invalid JSON. No changes applied.', flags: 1 << 6 })
					.catch(() => {});
			}
			...
			if (!v.ok) {
				return submitted
					.reply({
						content: `Validation failed: ${v.errors.slice(0, 5).join('; ')}${v.errors.length > 5 ? ` (+${v.errors.length - 5} more)` : ''}`,
						flags: 1 << 6,
					})
					.catch(() => {});
			}
			...
			if (_res_editmsg === 'ok') {
				return submitted
					.reply({
						content: 'Stored payload updated and deployed message edited successfully.',
						flags: 1 << 6,
					})
					.catch(() => {});
			}
			if (_res_editmsg === 'cleared') {
				return submitted
					.reply({
						content:
							'Stored payload updated but deployed message was missing/edited and references were cleared.',
						flags: 1 << 6,
					})
					.catch(() => {});
			}
			if (_res_editmsg === 'failed' || _res_editmsg === 'no-op') {
				return submitted
					.reply({
						content:
							'Stored payload updated. Could not apply to deployed message (no-op or error).',
						flags: 1 << 6,
					})
					.catch(() => {});
			}
		...
		if (id.startsWith('appmgr_app_deploy_')) {
			...
			const emb = new EmbedBuilder()
				.setTitle(app.name || `Application #${app.id}`)
				.setDescription(app.name || 'Apply using the button below.')
				.setColor(0x2f3136);
			safeAddField(emb, 'Questions', String((app.questions || []).length));
			if (app.submissionChannelId)
				safeAddField(emb, 'Submission Channel', `<#${app.submissionChannelId}>`);
			const row = new ActionRowBuilder().addComponents(
				new ButtonBuilder()
					.setCustomId(`apply_app_${app.id}`)
					.setLabel('Apply')
			...
			try {
				...
				const embedJSON = emb.data
					? emb.data
					: typeof emb.toJSON === 'function'
						? emb.toJSON()
						: undefined;
				updateApplication(app.id, {
					channelId: channel.id,
					messageId: sent.id,
					messageJSON: sanitizeMessageJSON({ embed: embedJSON, components: compsJSON }),
				});
				return interaction.reply({ content: 'Application posted.', flags: 1 << 6 }).catch(() => {});
			} catch (e) {
				try {
					require('../utils/logger').error('[applications] app deploy failed', {
						err: e && e.message,
					});
				} catch {}
				return interaction
					.reply({ content: 'Failed to post application.', flags: 1 << 6 })
					.catch(() => {});
			}
		}
		if (id === 'appmgr_apps_delete') {
			...
			if (!appId)
				return interaction
					.reply({ content: 'No application selected to delete.', flags: 1 << 6 })
					.catch(() => {});
			if (!getApplication(appId))
				return interaction
					.reply({ content: 'Application not found.', flags: 1 << 6 })
		...
		if (id.startsWith('appmgr_app_delete_') || id === 'appmgr_apps_delete') {
			...
			if (val !== 'DELETE')
				return submitted
					.reply({ content: 'Cancelled. Type DELETE to confirm.', flags: 1 << 6 })
					.catch(() => {});
			...
			try {
				...
				try {
					require('../utils/logger').error('[applications] update after delete failed', {
						err: e.message,
						stack: e.stack,
					});
				} catch {}
		...
		if (id.startsWith('appmgr_app_rename_')) {
			...
			if (_res === 'cleared' || _res === 'failed') {
				await submitted
					.reply({
						content:
							'Note: deployed application message could not be updated and was deregistered (check bot permissions / message availability).',
						flags: 1 << 6,
					})
					.catch(() => {});
			}
		...
		if (id.startsWith('appmgr_app_props_')) {
			...
			const row = buildNavRow([
				...
				semanticButton('danger', {
					id: `appprops_clearchan_${appId}`,
					label: 'ClrChan',
					enabled: !!app.submissionChannelId,
				}),
				semanticButton('primary', { id: `appprops_toggledm_${appId}`, label: 'DMs' }),
				backButton(`appprops_back_${appId}`, 'Back'),
			]);
		...
		if (session.data.view === 'appProps' && /^appprops_/.test(id)) {
			...
			if (action === 'setname') {
				...
				if (_res === 'cleared' || _res === 'failed')
					await sub
						.reply({
							content: 'Deployed message could not be updated and was deregistered.',
							flags: 1 << 6,
						})
						.catch(() => {});
			...
			if (action === 'setchan') {
				...
				if (_res_chan === 'cleared' || _res_chan === 'failed')
					await interaction
						.reply({
							content: 'Deployed message could not be updated and was deregistered.',
							flags: 1 << 6,
				...
				safeAddField(
					e2,
					'Submission Channel',
					interaction.channelId ? `<#${interaction.channelId}>` : '*none*',
				);
			...
			if (action === 'clearchan') {
				...
				return interaction
					.reply({ content: 'Submission channel cleared.', flags: 1 << 6 })
					.catch(() => {});
			}
			if (action === 'toggledm') {
				...
				if (_res_dm === 'cleared' || _res_dm === 'failed')
					await interaction
						.reply({
							content: 'Deployed message could not be updated and was deregistered.',
							flags: 1 << 6,
				...
				return interaction
					.reply({
						content: `DM responses now ${!app.dmResponses ? 'enabled' : 'disabled'}.`,
						flags: 1 << 6,
					})
					.catch(() => {});
			}
		...
		if (session.data.view === 'appRoles' && /^approles_/.test(id)) {
			...
			const app = getApplication(appId);
			if (!app || !key)
				return interaction.reply({ content: 'Invalid.', flags: 1 << 6 }).catch(() => {});
			const {
				ModalBuilder,
				TextInputBuilder,
				TextInputStyle,
			...
			const label =
				field === 'pending'
					? 'Single role ID or mention (or none to clear)'
					: 'Role IDs / mentions (space/comma separated)';
			const m = new ModalBuilder()
				.setCustomId(modalId)
				.setTitle(`Edit ${field}`)
				.addComponents(
			...
			if (/^\s*(none|null|-)\s*$/i.test(raw)) {
				...
				if (_res_clear === 'cleared' || _res_clear === 'failed')
					await submitted
						.reply({
							content: 'Deployed message could not be updated and was deregistered.',
							flags: 1 << 6,
				...
				if (!valid.length && tokens.length) {
					await submitted
						.reply({
							content: 'No valid role IDs found in input; no changes applied.',
							flags: 1 << 6,
						})
						.catch(() => {});
				} else {
					...
					if (_res_roles === 'cleared' || _res_roles === 'failed')
						await submitted
							.reply({
								content: 'Deployed message could not be updated and was deregistered.',
								flags: 1 << 6,
			...
			safeAddField(
				eRoles,
				'Pending',
				refreshed.pendingRole ? `<@&${refreshed.pendingRole}>` : '*none*',
			);
			...
			return submitted.update
				? submitted.update({ embeds: [eRoles], components: [headerRoles, r1, r2] })
				: submitted.reply({ content: 'Updated roles.', flags: 1 << 6 }).catch(() => {});
		}
		if (id.startsWith('appmgr_app_msgs_')) {
			...
			const e = createEmbed({
				title: `📨 Messages • App #${appId}`,
				description: app.name || 'Messages for this application',
			});
			safeAddField(
				e,
				'✅ Accept',
				app.acceptMessage ? `${app.acceptMessage.slice(0, 120)}` : '*(none)*',
				true,
			);
			safeAddField(
				e,
				'❌ Deny',
				app.denyMessage ? `${app.denyMessage.slice(0, 120)}` : '*(none)*',
				true,
			);
			safeAddField(
				e,
				'✉️ Confirm',
				app.confirmMessage ? `${app.confirmMessage.slice(0, 120)}` : '*(none)*',
				true,
			);
			safeAddField(
				e,
				'🏁 Completion',
				app.completionMessage ? `${app.completionMessage.slice(0, 120)}` : '*(none)*',
				true,
			);
			const row = buildNavRow([
				semanticButton('primary', { id: `appmsg_edit_accept_${appId}`, label: 'Accept' }),
		...
		if (session.data.view === 'appMsgs' && /^appmsg_(edit|back)_/.test(id)) {
			...
			if (!key)
				return interaction.reply({ content: 'Unknown field.', flags: 1 << 6 }).catch(() => {});
			const app = getApplication(appId);
			if (!app)
				return interaction.reply({ content: 'App missing.', flags: 1 << 6 }).catch(() => {});
			const {
			...
			if (_res_msg === 'cleared' || _res_msg === 'failed')
				await submitted
					.reply({
						content: 'Deployed message could not be updated and was deregistered.',
						flags: 1 << 6,
			...
			const refreshed = getApplication(appId);
			const eMsg = createEmbed({
				title: `Messages • App #${appId}`,
				description: 'Edit the various lifecycle messages.',
			});
			...
			return submitted.update
				? submitted.update({ embeds: [eMsg], components: [headerMsg, row2] })
				: submitted.reply({ content: 'Updated message.', flags: 1 << 6 }).catch(() => {});
		}
		...
		if (id === 'appmgr_back_apps') {
			...
			return interaction.update({
				embeds: [embed],
				components: buildApplicationsListComponents(page, totalPages),
			});
		}
		// Question list navigation / actions
		if (
			session.data.view === 'questions' &&
			/^appq_(prev|next|add|edit|del|reorder|back)_/.test(id)
		) {
			...
			if (action === 'prev' || action === 'next') {
				const perPage = 6;
				const totalPages = Math.max(1, Math.ceil(app.questions.length / perPage));
				let newPage = page + (action === 'next' ? 1 : -1);
				...
				return interaction.update({
					embeds: [embed],
					components: buildQuestionListComponents(appId, p, tp),
				});
			}
			if (action === 'back') {
				session.data.view = 'appDetail';
			...
			if (action === 'add') {
				...
				updateApplication(app.id, {
					questions: [...app.questions, { id: qid, type, label, required }],
				});
				const _res_addq = await tryEditDeployedApplication(app.id, interaction.client);
				if (_res_addq === 'cleared' || _res_addq === 'failed')
					await submitted
						.reply({
							content: 'Deployed message could not be updated and was deregistered.',
							flags: 1 << 6,
			...
			if (action === 'del') {
				if (!app.questions.length)
					return interaction.reply({ content: 'No questions.', flags: 1 << 6 }).catch(() => {});
				...
				if (_res_delq === 'cleared' || _res_delq === 'failed')
					await interaction
						.reply({
							content: 'Deployed message could not be updated and was deregistered.',
							flags: 1 << 6,
				...
				const {
					...
					page: p,
					totalPages: tp,
				} = buildQuestionListEmbed(
					appId,
					Math.min(
						session.data.qPage || 0,
						Math.max(0, Math.ceil((app.questions.length - 1) / 6) - 1),
					),
				);
				return interaction.update({
					embeds: [embed],
					...
					.addComponents(
						new ActionRowBuilder().addComponents(
							new TextInputBuilder()
								...
								.setStyle(TextInputStyle.Short)
								.setRequired(true)
								.setValue(target.label.slice(0, 100)),
						),
					)
					.addComponents(
						new ActionRowBuilder().addComponents(
			...
			if (action === 'reorder') {
				if (app.questions.length < 2)
					return interaction
						.reply({ content: 'Need at least 2 questions.', flags: 1 << 6 })
						.catch(() => {});
				...
				if (_res_reorder === 'cleared' || _res_reorder === 'failed')
					await interaction
						.reply({
							content: 'Deployed message could not be updated and was deregistered.',
							flags: 1 << 6,
		...
		if (id === 'appmgr_panels_delete') {
			...
			if (!panelId)
				return interaction
					.reply({ content: 'No panel selected to delete.', flags: 1 << 6 })
					.catch(() => {});
			if (!getPanel(panelId))
				return interaction.reply({ content: 'Panel not found.', flags: 1 << 6 }).catch(() => {});
		...
		if (id.startsWith('appmgr_panel_delete_') || id === 'appmgr_panels_delete') {
			...
			const panel = getPanel(panelId);
			if (!panel)
				return interaction.reply({ content: 'Panel missing.', flags: 1 << 6 }).catch(() => {});
			const {
				ModalBuilder,
				TextInputBuilder,
				TextInputStyle,
			...
			try {
				...
				try {
					require('../utils/logger').error('[applications] update after panel delete failed', {
						err: e.message,
						stack: e.stack,
					});
				} catch {}
				...
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							...
							.setStyle(TextInputStyle.Short)
							.setRequired(true)
							.setValue(panel.name.slice(0, 90)),
					),
				);
			await interaction.showModal(m);
			const sub = await interaction
				...
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							...
							.setStyle(TextInputStyle.Paragraph)
							.setRequired(false)
							.setValue((panel.description || '').slice(0, 400)),
					),
				);
			await interaction.showModal(m);
			const sub = await interaction
		...
		if (id.startsWith('appmgr_panel_channel_')) {
			...
			return interaction
				.reply({ content: 'Panel channel set to this channel.', flags: 1 << 6 })
				.catch(() => {});
		}
		if (id.startsWith('appmgr_panel_apps_')) {
			const panelId = id.split('_').pop();
			session.data.view = 'panelApps';
			session.data.panelId = panelId;
			const panel = getPanel(panelId);
			if (!panel)
				return interaction.reply({ content: 'Panel missing.', flags: 1 << 6 }).catch(() => {});
		...
		if (session.data.view === 'panelApps' && /^panelapps_/.test(id)) {
			...
			if (action === 'add' || action === 'remove') {
				...
				const m = new ModalBuilder()
					.setCustomId(modalId)
					.setTitle(action === 'add' ? 'Add Application' : 'Remove Application')
					.addComponents(
						new ActionRowBuilder().addComponents(
							new TextInputBuilder()
								.setCustomId('appid')
				...
				if (!appId)
					return submitted.reply({ content: 'Invalid ID.', flags: 1 << 6 }).catch(() => {});
				if (action === 'add') {
					if (!panel.applicationIds.includes(appId))
						updatePanel(panel.id, { applicationIds: [...panel.applicationIds, appId] });
					return submitted
						.reply({ content: 'Added application to panel.', flags: 1 << 6 })
						.catch(() => {});
				} else {
					if (panel.applicationIds.includes(appId))
						updatePanel(panel.id, {
							applicationIds: panel.applicationIds.filter((i) => i !== appId),
						});
					return submitted
						.reply({ content: 'Removed application.', flags: 1 << 6 })
						.catch(() => {});
				}
			}
		}
		if (id.startsWith('appmgr_panel_deploy_')) {
			...
			const apps = (panel.applicationIds || [])
				.map((id) => getApplication(id))
				.filter(Boolean)
				.filter((a) => a.enabled);
			const panelEmbed = createEmbed({
				title: panel.name || `Panel #${panel.id}`,
				description: panel.description || 'Select an application below.',
			});
			if (apps.length === 0) safeAddField(panelEmbed, 'Empty', 'No enabled applications linked.');
			else
				for (const app of apps)
					safeAddField(panelEmbed, `#${app.id} ${app.name}`, `${app.questions.length} question(s)`);
			...
			try {
				...
				const compsJSON = (sent.components || rows.slice(0, 5)).map((r) =>
					r.toJSON ? r.toJSON() : r,
				);
				updatePanel(panel.id, {
					channelId: channel.id,
					messageId: sent.id,
					messageJSON: sanitizeMessageJSON({ embed: panelEmbed.data, components: compsJSON }),
				});
				return interaction.reply({ content: 'Panel deployed.', flags: 1 << 6 }).catch(() => {});
			} catch (e) {
				try {
					require('../utils/logger').error('[applications] panel deploy failed', {
						err: e.message,
					});
				} catch {}
				return interaction
					.reply({ content: 'Failed to deploy panel.', flags: 1 << 6 })
					.catch(() => {});
			}
		}
		if (id === 'appmgr_back_panels') {
			...
			return interaction.update({
				embeds: [embed],
				components: buildPanelsListComponents(page, totalPages),
			});
		}
	} catch (err) {
		try {
		...
		if (interaction.isRepliable && !interaction.replied) {
			...
			interaction
				.reply({
					content: 'Error handling interaction. Details were logged to server logs.',
					flags: 1 << 6,
				})
				.catch(() => {});
		}
