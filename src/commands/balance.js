...
const progressBar = (current, max, size = 14) =>
	sharedProgressBar(current, max, size, { allowOverflow: true, showNumbers: false });

function buildBalancePayload(userId) {
	const { config } = require('../utils/storage');
	const cash =
		(config.testingMode ? cashUtils.getTestingCash(userId) : cashUtils.getCash(userId)) || 0;
	...
	embed.setFooter({
		text: `Deposit to grow your bank${config.testingMode ? ' • Testing Mode' : ''}. Taxes apply above the limit.`,
	});

	const row = buildNavRow([
		semanticButton('nav', {
			id: 'bank:menu:deposit',
			label: 'Deposit',
			emoji: theme.emojis.deposit,
		}),
		semanticButton('primary', {
			id: 'bank:menu:withdraw',
			label: 'Withdraw',
			emoji: theme.emojis.withdraw,
		}),
	]);
...
function buildDepositMenuPayload(userId) {
	...
	if (bank >= base) {
		lines.push(
			bank === base
				? 'At limit: further deposits will incur tax.'
				: 'Warning: Above limit – deposits incur heavy progressive tax.',
		);
	}
	const embed = createEmbed({
	...
	applyStandardFooter(embed, null, { testingMode: config.testingMode });
	embed.setFooter({
		text: `Choose Deposit Amount or Deposit Max. Back returns to Wallet.${config.testingMode ? ' • Testing Mode' : ''}`,
	});
	const row = buildNavRow([
		semanticButton('nav', {
...
function buildWithdrawMenuPayload(userId) {
	...
	applyStandardFooter(embed, null, { testingMode: config.testingMode });
	embed.setFooter({
		text: `Choose Withdraw Amount or Withdraw Max. Back returns to Wallet.${config.testingMode ? ' • Testing Mode' : ''}`,
	});
	const row = buildNavRow([
		semanticButton('nav', {
...
async function handleBalanceCommand(client, message) {
	...
	try {
		...
		const dropActive =
			activeDrops &&
			Array.from(activeDrops.values()).some((d) => d && !d.claimedBy && d.expiresAt > Date.now());
		if (dropActive) {
			...
			if (channel && channel.send) {
				...
				linkMsg = await channel
					.send({
						content: `🔍 Balance Check: <@${message.author.id}>`,
						...payload,
			...
			const jumpLink = linkMsg
				? `https://discord.com/channels/${linkMsg.guildId}/${linkMsg.channelId}/${linkMsg.id}`
				: null;
			await message
				.reply({
					content: jumpLink ? `Balance posted here → ${jumpLink}` : 'Balance posted.',
