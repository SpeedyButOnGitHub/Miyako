...
function buildDetailEmbed(guild, ev) {
	...
	let msgPreview = ev.message
		? ev.message.length > 300
			? ev.message.slice(0, 297) + '...'
			: ev.message
		: '(none)';
...
function detailRows(ev) {
	...
	const row = buildNavRow([
		semanticButton(ev.enabled ? 'danger' : 'success', {
			id: `events_toggle_${ev.id}`,
			label: ev.enabled ? 'Disable' : 'Enable',
			emoji: ev.enabled ? theme.emojis.disable : theme.emojis.enable,
		}),
		semanticButton('primary', {
			id: `events_edit_${ev.id}`,
			label: 'Edit',
			emoji: theme.emojis.edit || theme.emojis.message || '✏️',
		}),
		semanticButton('nav', {
			id: `events_notifs_${ev.id}`,
			label: 'Auto Msgs',
			emoji: theme.emojis.bell || '🔔',
		}),
		semanticButton('danger', {
			id: `events_delete_${ev.id}`,
			label: 'Delete',
			emoji: theme.emojis.delete,
		}),
	]);
