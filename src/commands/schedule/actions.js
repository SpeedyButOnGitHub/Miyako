...
async function ensureAnchor(interactionOrClient, ev, basePayloadOverride) {
	...
	try {
		if (
			/Midnight Bar/i.test(ev.name || '') &&
			/# The Midnight bar is opening:?$/im.test(baseContent)
		) {
			const { computeNextRange } = require('../../utils/timestampPlaceholders');
			const range = computeNextRange(ev);
			if (range)
				baseContent = baseContent.replace(
					/# The Midnight bar is opening:?$/im,
					`# The Midnight bar is opening in <t:${range.startSec}:R>`,
				);
		}
	} catch {}
	const payload = ev.messageJSON
		? { ...ev.messageJSON, content: baseContent }
		: { content: baseContent };
	...
	try {
		if (/Midnight Bar/i.test(ev.name || '')) {
			...
			const imageUrl =
				'https://media.discordapp.net/attachments/1385673109486571772/1402700465417883648/The_Midnight_Bar_Event_v1.png?ex=68c1085b&is=68bfb6db&hm=47ea205d83ebe96fee67b47e877093580bde85f5f906d10764cd05b3ab0e15af&=&format=webp&quality=lossless&width=1718&height=350';
			const { createEmbed } = require('../../utils/embeds');
			if (!payload.embeds) payload.embeds = [];
			const hasImage = payload.embeds.some(
				(e) => (e.data?.image?.url || e.image?.url) === imageUrl,
			);
	...
	if (!msg) {
		...
		if (msg)
			updateEvent(ev.id, {
				anchorChannelId: channel.id,
				anchorMessageId: msg.id,
				dynamicBaseContent: baseContent,
			});
	} else {
		let needsEdit = false;
		...
		if (expectedButtonId) {
			const hasButton =
				...
				msg.components.some((r) => r.components?.some?.((c) => c.customId === expectedButtonId));
...
async function manualTriggerAutoMessage(interaction, ev, notif) {
	...
	if (!targetChannelId && interaction)
		targetChannelId =
			interaction.channelId || (interaction.channel && interaction.channel.id) || null;
	...
	if (notif.isClockIn) {
		...
		try {
			ev.__clockIn.autoNext =
				ev.__clockIn.autoNext && typeof ev.__clockIn.autoNext === 'object'
					? ev.__clockIn.autoNext
					: {};
			...
			if (autoNextEntries.length) {
				for (const [userId, roleKey] of autoNextEntries) {
					if (!roleKey || !displayPositions[roleKey]) {
		...
		try {
			...
			const hydrated = {
				...ev,
				__clockIn: { ...(ev.__clockIn || {}), positions: displayPositions },
			};
			...
			const prevMessageIds = Array.isArray(ev.__clockIn.messageIds)
				? ev.__clockIn.messageIds.slice()
				: [];
			...
			if (sentClock) {
				...
				try {
					...
					const rec =
						map[notif.id] && typeof map[notif.id] === 'object'
							? { ...map[notif.id] }
							: { channelId: channel.id, ids: [] };
				...
				try {
					try {
						...
						scheduleDeleteForMessage(interaction.client, channel, sentClock, notif, ev);
					} catch {}
				} catch {}
				...
				if (!config.testingMode) {
					...
					if (!ev.__clockIn.messageIds.includes(sentClock.id))
						ev.__clockIn.messageIds.push(sentClock.id);
					if (ev.__clockIn.messageIds.length > 10)
						ev.__clockIn.messageIds = ev.__clockIn.messageIds.slice(-10);
					updateEvent(ev.id, {
						autoMessages: ev.autoMessages,
						__clockIn: ev.__clockIn,
						__notifMsgs: ev.__notifMsgs,
					});
					...
					try {
						...
						for (const mid of prevMessageIds || []) {
							...
							try {
								...
								if (
									...
									oldMsg.author &&
									oldMsg.author.id &&
									oldMsg.author.id === interaction.client.user?.id &&
									typeof oldMsg.delete === 'function'
								) {
									try {
										const { newCorrelationId } = require('../../utils/correlation');
										const corrId = newCorrelationId();
										require('../../utils/logger').info(
											'[manualTrigger] deleting previous clock-in message',
											{ mid, eventId: ev.id, notifId: notif.id, correlationId: corrId },
										);
									} catch {}
						...
						ev.__clockIn.messageIds = (ev.__clockIn.messageIds || []).filter(
							(id) => id && id === sentClock.id,
						);
	...
	try {
		...
		if (!(notif && Number.isFinite(notif.deleteAfterMs) && Number(notif.deleteAfterMs) > 0)) {
			...
			try {
				try {
					const { scheduleDeleteForMessage } = require('./notifications');
					scheduleDeleteForMessage(interaction.client, channel, sent, notif, ev);
				} catch {}
			} catch {}
		}
	} catch {}
