...
const {
	getAll: getPersistedDeletes,
	setForMessage: persistDeleteForMessage,
	removeForMessage: removePersistedDelete,
} = require('../../utils/scheduledDeletes');
...
function buildNotifsEmbed(guild, ev) {
	...
	safeAddField(
		embed,
		'Days',
		(ev.days || []).map((d) => DAY_NAMES[d] || d).join(', ') || 'All',
		true,
	);
		...
		const lines = list
			...
			.map((m) => {
				...
				if (m.messageJSON) {
					...
					else if (Array.isArray(m.messageJSON.embeds) && m.messageJSON.embeds.length)
						preview = (
							m.messageJSON.embeds[0].title ||
							m.messageJSON.embeds[0].description ||
							'(embed)'
						)
							.toString()
							.slice(0, 60);
				...
				const ttlDisp = hasOwn
					? m.deleteAfterMs > 0
						? humanizeMs(m.deleteAfterMs)
						: 'off'
					: 'off';
				const ttlLabel = ` [TTL ${ttlDisp}]`;
				const mentionNote =
					Array.isArray(m.mentions) && m.mentions.length ? ` [@${m.mentions.length}]` : '';
...
function notifManagerRows(ev) {
	const row = buildNavRow([
		semanticButton('success', {
			id: `event_notif_add_${ev.id}`,
			label: 'Add',
			emoji: theme.emojis.create || '➕',
		}),
		semanticButton('primary', {
			id: `event_notif_selectmode_${ev.id}`,
			label: 'Select',
			emoji: theme.emojis.events || '📋',
			enabled: !!(ev.autoMessages || []).length,
		}),
	]);
	return [row];
}
...
function notifSelectRows(ev) {
	...
	const opts = raw.map((n) => {
		...
		const opt = {
			label: `${humanizeMinutes(n.offsetMinutes)} ${n.enabled ? '(on)' : '(off)'} #${n.id}`.slice(
				0,
				100,
			),
			value: n.id,
	...
	const row1 = new ActionRowBuilder().addComponents(
		new StringSelectMenuBuilder()
			.setCustomId(`event_notif_select_${ev.id}`)
			.setPlaceholder('Select auto message')
			.addOptions(opts),
	);
	return [row1];
}
...
function notifDetailRows(ev, notif) {
	const row1 = buildNavRow([
		semanticButton(notif.enabled ? 'danger' : 'success', {
			id: `event_notif_toggle_${ev.id}_${notif.id}`,
			label: notif.enabled ? 'Disable' : 'Enable',
			emoji: notif.enabled ? theme.emojis.disable : theme.emojis.enable,
		}),
		semanticButton('primary', {
			id: `event_notif_edit_${ev.id}_${notif.id}`,
			label: 'Edit',
			emoji: theme.emojis.edit || theme.emojis.message || '✏️',
		}),
	]);
	const row2 = buildNavRow([
		semanticButton('success', {
			id: `event_notif_trigger_${ev.id}_${notif.id}`,
			label: 'Trigger',
			emoji: theme.emojis.enable || '✅',
		}),
		semanticButton('danger', {
			id: `event_notif_delete_${ev.id}_${notif.id}`,
			label: 'Delete',
			emoji: theme.emojis.delete || '🗑️',
		}),
	]);
	return [row1, row2];
}
...
function scheduleDeleteForMessage(client, channel, msg, notif, ev) {
	try {
		...
		if (remaining <= 0) {
			try {
				if (channel && channel.messages) channel.messages.delete(msg.id).catch(() => {});
			} catch {}
			try {
				if (ev && ev.id && notif && notif.id) {
					const map =
						ev.__notifMsgs && typeof ev.__notifMsgs === 'object' ? { ...ev.__notifMsgs } : {};
					const rec =
						map[notif.id] && typeof map[notif.id] === 'object' ? { ...map[notif.id] } : null;
					...
					if (ev.__clockIn && Array.isArray(ev.__clockIn.messageIds))
						ev.__clockIn.messageIds = ev.__clockIn.messageIds.filter((id) => id && id !== msg.id);
					updateEvent(ev.id, { __notifMsgs: map, __clockIn: ev.__clockIn });
				}
			} catch {}
			try {
				removePersistedDelete(msg.id);
			} catch {}
			try {
				require('../../utils/logger').info('[scheduleDelete] deleted immediately', {
					mid: msg.id,
					eventId: ev && ev.id ? ev.id : null,
					notifId: notif && notif.id ? notif.id : null,
		...
		const t = setTimeout(async () => {
			try {
				scheduledDeleteTimers.delete(msg.id);
			} catch {}
			try {
				const ch =
					channel ||
					(client && client.channels
						? await client.channels.fetch(msg.channelId).catch(() => null)
						: null);
				if (ch && ch.messages) {
					try {
						require('../../utils/logger').info('[scheduleDelete] deleting message', {
							mid: msg.id,
							eventId: ev && ev.id ? ev.id : null,
							notifId: notif && notif.id ? notif.id : null,
							correlationId: corrId,
		...
		try {
			const entry = {
				eventId: ev && ev.id ? ev.id : null,
				notifId: notif && notif.id ? notif.id : null,
				channelId: channel && channel.id ? channel.id : msg.channelId || null,
				scheduledAt: Date.now(),
				deleteAfterMs: delMs,
				messageCreatedAt: msg.createdTimestamp || Date.now(),
				correlationId: corrId,
			};
			try {
				persistDeleteForMessage(msg.id, entry);
			} catch (e) {
				logger &&
					logger.warn &&
					logger.warn('[scheduleDelete] persist failed', { err: e && e.message });
			}
		} catch (e) {
			logger &&
				logger.warn &&
				logger.warn('[scheduleDelete] prepare persist failed', { err: e && e.message });
		}
	} catch (e) {
		try {
			require('../../utils/logger').warn('[scheduleDelete] failed', { err: e && e.message });
		} catch {}
	}
}
	...
	try {
		...
		for (const [mid, entry] of Object.entries(persisted)) {
			try {
				...
				let notif = null;
				try {
					if (entry.eventId) ev = getEvent(entry.eventId);
				} catch {}
				try {
					if (ev && ev.autoMessages && entry.notifId)
						notif = (ev.autoMessages || []).find((n) => String(n.id) === String(entry.notifId));
				} catch {}
				if (!notif) notif = { deleteAfterMs: entry.deleteAfterMs };
				// Schedule using existing helper
				try {
					scheduleDeleteForMessage(client, ch, msg, notif, ev);
				} catch {}
			} catch {}
		}
	} catch (e) {
		try {
			require('../../utils/logger').warn('[reconstructScheduledDeletes] failed', {
				err: e && e.message,
			});
		} catch {}
	}
...
async function refreshTrackedAutoMessages(client, ev, options = {}) {
	try {
			...
			for (const notif of ev.autoMessages) {
				...
				if (notif.messageJSON && typeof notif.messageJSON === 'object') {
					...
					if (!base.content && !base.embeds)
						base.content = notif.message || `Auto message (${ev.name})`;
				...
				try {
					const shouldForce =
						...
						(options.forceAll === true ||
							(Array.isArray(options.forceForIds) &&
								options.forceForIds.map(String).includes(String(notif.id))));
					...
					if (latestMsg) {
						...
						if (!shouldForce && liveSig !== newSig) {
							...
							if ((latestMsg.embeds || []).length) {
								payload.embeds = latestMsg.embeds
									.map((e) => ({
										title: e.title,
						...
						try {
							if (
								...
								payload &&
								payload.content &&
								!payload.content.startsWith(mentionLine)
							) {
								payload.content = `${mentionLine}\n${payload.content}`.slice(0, 2000);
								payload.allowedMentions = payload.allowedMentions || {
									roles: notif.mentions.slice(0, 20),
								};
							}
						} catch {}
					}
				...
				for (const mid of rec.ids.slice(-3)) {
					try {
						...
						if (msg) {
							...
							await retry(() => msg.edit(payload), { attempts: 3, baseMs: 50, maxMs: 300 }).catch(
								(e) => {
									try {
										logger &&
											logger.warn &&
											logger.warn('[notif refresh] edit failed', {
												err: e?.message,
												mid,
												eventId: ev.id,
											});
							...
							try {
								scheduleDeleteForMessage(client, channel, msg, notif, ev);
							} catch {}
						}
					} catch (e) {
						try {
							logger &&
								logger.warn &&
								logger.warn('[notif refresh] fetch/edit failed', {
									err: e.message,
									mid,
									eventId: ev.id,
								});
		...
		if (
			ev.__clockIn &&
			((Array.isArray(ev.__clockIn.messageIds) && ev.__clockIn.messageIds.length) ||
				(ev.__notifMsgs && typeof ev.__notifMsgs === 'object'))
		) {
			const msgTargets = [];
			try {
						...
						try {
							...
							if (rec && Array.isArray(rec.ids)) {
								for (const id of rec.ids)
									msgTargets.push({
										id,
										channelId: rec.channelId || ev.__clockIn?.channelId || ev.channelId,
									});
							}
						} catch {}
					}
			...
			const seen = new Set();
			const uniqueTargets = msgTargets
				.filter((t) => {
			...
			for (const t of uniqueTargets) {
				try {
						...
						if (rt.__clockIn && rt.__clockIn.positions) {
							...
							if (guild) {
								...
								const changed = Object.entries(prunedPositions).some(([k, v]) => {
									const orig =
										(ev.__clockIn && ev.__clockIn.positions && ev.__clockIn.positions[k]) || [];
								...
								if (changed) {
									...
									try {
										logger &&
											logger.debug &&
											logger.debug('[ClockIn Hydrate]', { eventId: ev.id, updated: true });
									} catch {}
								}
								hydrated = {
									...ev,
									__clockIn: { ...ev.__clockIn, positions: rt.__clockIn.positions },
								};
							}
						}
					} catch {}
					...
					try {
						mentionLine =
							Array.isArray(clkNotif?.mentions) && clkNotif.mentions.length
								? clkNotif.mentions.map((r) => `<@&${r}>`).join(' ')
								: null;
					} catch {}
					...
					if (existingSig !== nextSig) {
						try {
							...
							if (mentionLine && Array.isArray(clkNotif?.mentions))
								payload.allowedMentions = { roles: clkNotif.mentions.slice(0, 20) };
							await retry(() => msg.edit(payload), { attempts: 3, baseMs: 50, maxMs: 300 }).catch(
								(e) => {
									try {
										logger &&
											logger.warn &&
											logger.warn('[notif refresh] clockin edit failed', {
												err: e?.message,
												mid: t.id,
												eventId: ev.id,
											});
							...
							try {
								scheduleDeleteForMessage(client, channel, msg, clkNotif, ev);
							} catch (e) {
								logger &&
									logger.warn &&
									logger.warn('[notif refresh] schedule failed', { err: e && e.message });
							}
						} catch (e) {
							try {
								logger &&
									logger.warn &&
									logger.warn('[notif refresh] clockin edit outer failed', {
										err: e?.message,
										mid: t.id,
										eventId: ev.id,
									});
		...
		try {
			logger &&
				logger.warn &&
				logger.warn('[refreshTrackedAutoMessages] failed', { err: e?.message });
		} catch {}
	}
}
