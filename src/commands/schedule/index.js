...
function visibleEvents() {
	...
	if (config && config.testingMode)
		return events.filter(
			(e) => !(e.__testEvent || (typeof e.name === 'string' && e.name.startsWith('CI Test'))),
		);
	return events;
}
...
async function handleEventCreateModal(interaction) {
	if (!interaction.isModalSubmit() || !interaction.customId.startsWith('event_create_modal_'))
		return;
	...
	if (!/^\d{1,32}$/.test(channelId))
		return safeReply(interaction, { content: '❌ Invalid channel id.', flags: 1 << 6 });
	...
	try {
		await ensureAnchor(interaction, ev, { content: base });
	} catch {}
	const jsonNote = messageJSON ? ' (JSON payload detected)' : '';
	await interaction
		.reply({
			content: `✅ Event ${ev.name} created with ${ev.times.length} time(s).${jsonNote}`,
			flags: 1 << 6,
		})
		.catch(() => {});
	if (managerMessageId) {
		try {
			...
			if (mgrMsg) {
				...
				await mgrMsg
					.edit({
						embeds: [buildMainEmbed(interaction.guild, events)],
						components: mainRows(events),
					})
					.catch(() => {});
			}
...
async function handleScheduleModal(interaction) {
	if (!interaction.isModalSubmit() || !interaction.customId.startsWith('schedule_create_modal'))
		return;
	await interaction
		.reply({ content: 'Scheduling system deprecated. Use Events Manager.', flags: 1 << 6 })
		.catch(() => {});
}
...
async function handleEventEditModal(interaction) {
	...
	if (!/^\d+$/.test(eventId)) {
		await interaction.reply({ content: '❌ Bad event id.', flags: 1 << 6 }).catch(() => {});
		return;
	}
	const ev = getEvent(eventId);
	if (!ev) {
		await interaction.reply({ content: 'Event not found.', flags: 1 << 6 }).catch(() => {});
		return;
	}
	let updatedEv = null;
	if (interaction.customId.startsWith('event_times_modal_')) {
		...
		const ranges = times
			...
			.filter(Boolean);
		if (!times.length) {
			await interaction.reply({ content: '❌ Provide times.', flags: 1 << 6 }).catch(() => {});
			return;
		}
		...
		const days = raw
			...
			.filter((d) => d !== undefined);
		if (!days.length) {
			await interaction.reply({ content: '❌ Invalid days.', flags: 1 << 6 }).catch(() => {});
			return;
		}
		...
		await interaction
			.reply({
				content: `✅ Message updated${messageJSON ? ' (JSON payload detected)' : ''}.`,
				flags: 1 << 6,
			})
			.catch(() => {});
		...
		if (!/^\d{1,32}$/.test(channelId))
			return safeReply(interaction, { content: '❌ Invalid channel id.', flags: 1 << 6 });
		if (!times.length)
			return safeReply(interaction, { content: '❌ Provide times.', flags: 1 << 6 });
		if (!days.length) return safeReply(interaction, { content: '❌ Invalid days.', flags: 1 << 6 });
		let messageJSON = null;
		const cleaned = messageContent
			.replace(/^```(json)?/i, '')
		...
		updatedEv = updateEvent(ev.id, {
			...
			message: messageContent,
			messageJSON,
			dynamicBaseContent: messageJSON?.content || messageContent,
		});
		try {
			await ensureAnchor(interaction, updatedEv, {
				content: messageJSON?.content || messageContent,
			});
		} catch {}
		await interaction
			.reply({
				content: `✅ Event updated${messageJSON ? ' (JSON payload detected)' : ''}.`,
				flags: 1 << 6,
			})
			.catch(() => {});
	}
	if (managerMessageId && updatedEv) {
		try {
			const mgrMsg = await interaction.channel.messages.fetch(managerMessageId).catch(() => null);
			if (mgrMsg) {
				const isDetail = mgrMsg.components.some((r) =>
					r.components.some((c) => c.customId === `events_toggle_${updatedEv.id}`),
				);
				if (isDetail) {
					await mgrMsg
						.edit({
							embeds: [buildDetailEmbed(interaction.guild, updatedEv)],
							components: detailRows(updatedEv),
						})
						.catch(() => {});
				} else {
					const events = getEvents();
...
async function handleEventNotificationModal(interaction) {
	...
	if (kind === 'add') {
		...
		let mentionsRaw = '';
		try {
			mentionsRaw = (interaction.fields.getTextInputValue('mentions') || '').trim();
		} catch {}
		const msgChannelId = chanRaw.replace(/[<#>]/g, '');
		if (msgChannelId && !/^\d{1,32}$/.test(msgChannelId)) {
			return safeReply(interaction, { content: '❌ Invalid channel id.', flags: 1 << 6 });
		}
		let messageJSON = null;
		...
		await interaction
			.reply({
				content: `✅ Auto message #${nextId} created${messageJSON ? ' (JSON)' : ''}.`,
				flags: 1 << 6,
			})
			.catch(() => {});
	} else if (kind === 'edit') {
		...
		if (cleanedChan && !/^\d{1,32}$/.test(cleanedChan))
			return safeReply(interaction, { content: '❌ Invalid channel id.', flags: 1 << 6 });
		const offsetRaw = interaction.fields.getTextInputValue('offset');
		const offset = parseOffsetInput(offsetRaw);
		const deleteAfterRaw = interaction.fields.getTextInputValue('deleteafter');
		...
		const newList = list.map((entry) => {
			...
			if (String(e.id) === String(_notifId)) {
				...
				if (cleanedChan) e.channelId = cleanedChan;
				else delete e.channelId;
				if (mentionsRaw) {
					const roleIds = Array.from(
						new Set(
							mentionsRaw
					...
					if (roleIds.length) e.mentions = roleIds;
					else delete e.mentions;
				} else {
		...
		await interaction
			.reply({ content: '✅ Auto messages updated for this event.', flags: 1 << 6 })
			.catch(() => {});
		try {
			await refreshTrackedAutoMessages(interaction.client, updatedEv, { forceForIds: [_notifId] });
		} catch {}
	}
	if (managerMessageId && updatedEv) {
		try {
			...
			if (mgrMsg) {
				if (
					mgrMsg.components.some((r) =>
						r.components.some((c) => c.customId?.startsWith('event_notif_')),
					)
				) {
					await mgrMsg
						.edit({
							embeds: [buildNotifsEmbed(interaction.guild, updatedEv)],
							components: notifManagerRows(updatedEv),
						})
						.catch(() => {});
				} else if (
					mgrMsg.components.some((r) =>
...
async function handleEventButtonInteraction(interaction) {
	try {
		...
		try {
			const res = await ActiveMenus.processInteraction(interaction).catch(() => ({
				handled: false,
			}));
		...
		if (id.startsWith('event_notify_')) {
			...
			try {
				await interaction.reply({
					content: '🔔 Notification buttons are managed by the Events Manager.',
					flags: 1 << 6,
				});
			} catch {}
			return;
		...
		try {
			await interaction.reply({
				content: 'Button pressed (no-op in this environment).',
				flags: 1 << 6,
			});
		} catch {}
	} catch (e) {
		try {
			await interaction.reply({
				content: 'An error occurred handling the schedule button.',
				flags: 1 << 6,
			});
		} catch {}
	}
	...
	try {
		...
		if (customId === 'events_create') {
			const modal = new ModalBuilder()
				.setCustomId(`event_create_modal_${interaction.message.id}`)
		...
		if (customId === 'events_select_mode') {
			...
			await interaction.update({
				embeds: [buildMainEmbed(interaction.guild, events)],
				components: buildSelectRows('select', events),
			});
			session.data = data;
			return;
		}
		if (customId === 'events_delete_mode') {
			...
			await interaction.update({
				embeds: [buildMainEmbed(interaction.guild, events)],
				components: buildSelectRows('delete', events),
			});
			session.data = data;
			return;
		}
		...
		if (customId.startsWith('events_toggle_')) {
			...
			if (!ev) return interaction.reply({ content: 'Missing event.', flags: 1 << 6 });
			const updated = updateEvent(id, { enabled: !ev.enabled });
			await interaction.update({
				embeds: [buildDetailEmbed(interaction.guild, updated)],
				components: detailRows(updated),
			});
			return;
		}
		if (customId.startsWith('events_edit_')) {
			...
			if (!ev) return interaction.reply({ content: 'Missing event.', flags: 1 << 6 });
			const modal = new ModalBuilder()
				.setCustomId(`event_edit_modal_${id}_${interaction.message.id}`)
				.setTitle('Edit Event')
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
					...
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							...
							.setStyle(TextInputStyle.Short)
							.setRequired(true)
							.setValue(ev.channelId || ''),
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('times')
							.setLabel('Times (HH:MM or HH:MM-HH:MM, comma)')
							.setStyle(TextInputStyle.Paragraph)
							.setRequired(true)
							.setValue((ev.times || []).join(',')),
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('days')
							...
							.setStyle(TextInputStyle.Short)
							.setRequired(true)
							.setValue((ev.days || []).map((d) => DAY_NAMES[d] || d).join(',')),
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('message')
							...
							.setRequired(true)
							.setValue(
								ev.message || (ev.messageJSON ? JSON.stringify(ev.messageJSON, null, 2) : ''),
							),
					),
				);
			await interaction.showModal(modal);
		...
		if (customId.startsWith('events_notifs_')) {
			...
			await interaction.update({
				embeds: [buildNotifsEmbed(interaction.guild, getEvent(id))],
				components: notifManagerRows(ev),
			});
			return;
		}
		if (customId.startsWith('event_notif_back_')) {
			...
			if (!ev) return interaction.reply({ content: 'Missing event.', flags: 1 << 6 });
			await interaction.update({
				embeds: [buildDetailEmbed(interaction.guild, ev)],
				components: detailRows(ev),
			});
			return;
		}
		if (customId.startsWith('event_notif_add_')) {
			...
			if (!ev) return interaction.reply({ content: 'Missing event.', flags: 1 << 6 });
			const modal = new ModalBuilder()
				.setCustomId(`notif_add_modal_${id}_${interaction.message.id}`)
				.setTitle('Add Auto Message')
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('channel')
							.setLabel('Channel ID (blank=event)')
							.setStyle(TextInputStyle.Short)
							.setRequired(false)
							.setPlaceholder(ev.channelId || ''),
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('offset')
							.setLabel('When before start? (e.g. 15m, 1h, 2h30m)')
							.setStyle(TextInputStyle.Short)
							.setRequired(true)
							.setValue('5m'),
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('deleteafter')
							.setLabel('Delete after (e.g. 10m, 0=disable)')
							.setStyle(TextInputStyle.Short)
							.setRequired(true)
							.setValue('0'),
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							...
							.setLabel('Role IDs to ping (comma/space)')
							.setStyle(TextInputStyle.Short)
							.setRequired(false)
		...
		if (customId.startsWith('event_notif_edit_')) {
			...
			const suggestTTL =
				currentTTL <= 0
					? '0'
					: currentTTL % 3600000 === 0
						? `${Math.floor(currentTTL / 3600000)}h`
						: currentTTL % 60000 === 0
							? `${Math.floor(currentTTL / 60000)}m`
							: `${Math.max(1, Math.floor(currentTTL / 1000))}s`;
			const modal = new ModalBuilder()
				.setCustomId(`notif_edit_modal_${evId}_${notifId}_${interaction.message.id}`)
				.setTitle('Edit Auto Message')
				.addComponents(
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('channel')
							...
							.setStyle(TextInputStyle.Short)
							.setRequired(false)
							.setValue(notif.channelId || ''),
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('offset')
							.setLabel('When before start? (e.g. 10m, 2h)')
							.setStyle(TextInputStyle.Short)
							.setRequired(true)
							.setValue(notif.offsetMinutes ? `${notif.offsetMinutes}m` : 'start'),
					),
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							.setCustomId('deleteafter')
							.setLabel('Delete after (e.g. 10m, 2h, 0=disable)')
							.setStyle(TextInputStyle.Short)
							.setRequired(true)
							.setValue(suggestTTL),
					),
					...
					new ActionRowBuilder().addComponents(
						new TextInputBuilder()
							...
							.setValue(
								notif.message ||
									(notif.messageJSON ? JSON.stringify(notif.messageJSON, null, 2) : ''),
							),
					),
				);
			await interaction.showModal(modal);
		...
		if (customId.startsWith('event_notif_selectmode_')) {
			...
			if (!ev) return interaction.reply({ content: 'Missing event.', flags: 1 << 6 });
			await interaction.update({
				embeds: [buildNotifsEmbed(interaction.guild, ev)],
				components: notifSelectRows(ev),
			});
			return;
		}
		...
		if (customId.startsWith('event_notif_toggle_')) {
			...
			await interaction.update({
				embeds: [buildNotifsEmbed(interaction.guild, getEvent(evId))],
				components: notifDetailRows(ev, list[idx]),
			});
			return;
		}
		if (customId.startsWith('event_notif_trigger_')) {
			...
			try {
				...
				await interaction
					.reply({
						content: sentOk
							? `✅ Triggered auto message #${notif.id}${config.testingMode ? ' (testing mode output only)' : ''}.`
				...
				await interaction
					.reply({ content: '❌ Error: ' + (e.message || e), flags: 1 << 6 })
					.catch(() => {});
			}
			return;
		}
		...
		if (interaction.isStringSelectMenu()) {
			if (customId === 'events_select') {
				...
				const ev = getEvent(id);
				if (!ev) return interaction.reply({ content: 'Not found.', flags: 1 << 6 });
				data.mode = 'detail';
				data.currentId = id;
				await interaction.update({
					embeds: [buildDetailEmbed(interaction.guild, ev)],
			...
			if (customId.startsWith('event_notif_select_')) {
				...
				await interaction.update({
					embeds: [buildNotifsEmbed(interaction.guild, ev)],
					components: notifDetailRows(ev, notif),
				});
				return;
			}
		}
		...
		const genErrorCode = () => {
			try {
				...
				return `EVT-${Math.floor(Math.random() * 9000) + 1000}`;
			}
		};
		...
		try {
			await safeReply(interaction, {
				content: `An error occurred processing the events menu. (${errorCode})`,
				flags: 1 << 6,
			});
		} catch {}
		...
		try {
			...
			try {
				require('../../utils/logger').error('[events handler] (logging failed)', {
		...
		try {
			...
			try {
				require('../../utils/logger').error('[events handler] (post to channel failed)', {
					err: e.message,
				});
			} catch {}
		}
