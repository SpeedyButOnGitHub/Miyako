...
async function handlePurgeCommand(client, message, args) {
	...
	if (isBlacklisted(message.channelId)) {
		return message.reply({
			content: '❌ This channel is blacklisted for purge operations.',
			allowedMentions: { repliedUser: false },
		});
	}
	...
	if (!count || count <= 0) {
		return message.reply({
			content: 'Usage: `.purge <amount 1-100> [@user]`',
			allowedMentions: { repliedUser: false },
		});
	}
	const { max } = getLimits();
	if (count > max) {
		return message.reply({
			content: `❌ Amount too large. Max allowed: ${max}.`,
			allowedMentions: { repliedUser: false },
		});
	}
	...
	if (needsConfirm) {
		...
		ActiveMenus.registerMessage(sent, {
			...
			data: { count, userFilter, contains, bots, images },
		});
		return;
	}
...
async function executePurge(client, message, count, userFilter, filters = {}) {
	try {
		...
		if (!toDelete || toDelete.length === 0) {
			return message.reply({
				content: 'Nothing to delete (messages may be too old or no matches).',
				allowedMentions: { repliedUser: false },
			});
		}
		let deletedCount = 0;
		...
		try {
			...
			await sendModLog(
				...
				message.author,
				'purged',
				`${deletedCount} message(s)${filterStr}`,
				true,
				null,
				null,
			);
		...
		try {
			await message.reply({ embeds: [resultEmbed], allowedMentions: { repliedUser: false } });
		} catch {}
	} catch (err) {
		try {
			require('../../utils/logger').error('[purge] error', { err: err.message });
		} catch {}
		try {
			await message.reply({
				content: `Error during purge: ${err.message || err}`,
				allowedMentions: { repliedUser: false },
			});
		} catch {}
	}
