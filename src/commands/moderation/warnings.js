...
function maybeSeedTestingData(guild) {
	...
	for (const id of chosen) {
		...
		for (let j = 0; j < n; j++) {
			warns.push({
				moderator: id,
				reason: ['Spam', 'Off-topic', 'Rude language', 'NSFW', 'Disrespect'][
...
function buildDashboardEmbed(guild, page) {
	...
	const all = [...usersSet]
		.map((userId) => ({ userId, count: getVisibleWarnings(userId).length }))
		.filter((x) => x.count > 0)
	...
	applyFooterWithPagination(embed, guild, {
		...
		page: curPage,
		totalPages,
		extra: `Mute at ${muteT} • Kick at ${kickT}`,
	});
	...
	rows.push(
		new ActionRowBuilder().addComponents(
			new StringSelectMenuBuilder()
				...
				.setMaxValues(1)
				.addOptions(
					userOptions.length ? userOptions : [{ label: 'No users', value: 'noop', default: true }],
				)
...
function buildUserView(guild, userId, page = 1, opts = {}) {
	...
	const baseDesc = total
		? chunk.map((e, i) => formatWarnLine(guild, e, start + i + 1)).join('\n\n')
		: 'This user has no warnings.';
	const nxt = getNextPunishmentInfo(total);
	const disclaimer = nxt
		? `${nxt.remaining} warning${nxt.remaining === 1 ? '' : 's'} remaining until ${nxt.label}`
		: null;
	const { muteT, kickT } = getThresholds();
	const embed = new EmbedBuilder()
	...
	applyFooterWithPagination(embed, guild, {
		...
		totalPages,
		extra: `Total: ${total} • Mute at ${muteT} • Kick at ${kickT}`,
	});
	...
	const actions = new ActionRowBuilder().addComponents(
		...
		new ButtonBuilder()
			.setCustomId(`warns:remove:${userId}`)
	...
	try {
		if (interaction.isButton()) {
			...
			if (id.startsWith('warns:dash:')) {
				...
				if (m) {
					...
					if (action === 'prev' || action === 'next') {
						...
						await interaction
							.update({ embeds: [dash.embed], components: dash.rows })
							.catch(() => {});
					} else {
			...
			if (id.startsWith('warns:user:')) {
				...
				if (m) {
					...
					if (action === 'prev' || action === 'next') {
						...
						await interaction
							.update({ embeds: [view.embed], components: view.rows })
							.catch(() => {});
					} else {
						await interaction.deferUpdate().catch(() => {});
					}
			...
			if (id.startsWith('warns:add:')) {
				...
				const modal = new ModalBuilder().setCustomId(`warns:add:${userId}`).setTitle('Add warning');
				modal.addComponents(
					new ActionRowBuilder().addComponents(
		...
		if (interaction.isStringSelectMenu() && interaction.customId.startsWith('warns:')) {
			...
			if (id.startsWith('warns:removePick:')) {
				...
				if (config.testingMode && (!Array.isArray(arr) || arr.length === 0)) {
					...
					arr = Array.isArray(vis)
						? vis.map((x) => ({ moderator: x.moderator, reason: x.reason, date: x.date }))
						: [];
				}
				const removed = [];
				...
				let target =
					interaction.guild.members.cache.get(userId) || interaction.client.users.cache.get(userId);
				if (!target) target = await interaction.client.users.fetch(userId).catch(() => null);
				try {
					if (target) await sendUserDM(target, actionText, null, null, null);
				} catch {}
				try {
					if (target)
						await sendModLog(
				...
				try {
					await interaction.followUp({
						content: `${EMOJI_SUCCESS} Removed ${count} warning${count === 1 ? '' : 's'}.`,
						flags: 1 << 6,
					});
				} catch {}
				return;
		...
		if (interaction.isModalSubmit() && interaction.customId.startsWith('warns:add:')) {
			...
			try {
				...
				if (msg) {
					entry.logMsgId = msg.id;
					entry.logChannelId = msg.channelId;
					saveConfig();
				}
			} catch {}
			...
			try {
				...
				if (interaction.message && interaction.message.edit) {
					await interaction.message
						.edit({ embeds: [view.embed], components: view.rows })
						.catch(() => {});
				...
				await interaction
					.reply({ content: `${EMOJI_SUCCESS} Warning added.`, flags: 1 << 6 })
					.catch(() => {});
			}
			return;
		}
		...
		try {
			require('../../utils/logger').error('[Warnings Interaction Error]', { err: err.message });
		} catch {}
		try {
