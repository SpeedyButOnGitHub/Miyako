...
async function sendUserDM(target, action, duration = null, reason = null, extra = null) {
	...
	if (!al.startsWith('warning removed') && reason)
		embed.addFields({ name: '📝 Reason', value: reason, inline: true });
	if (duration)
		embed.addFields({ name: `${theme.emojis.duration} Duration`, value: duration, inline: true });
	...
	try {
		...
		try {
			require('../../utils/logger').warn('[DM Error] Could not DM user', {
				user: tag,
				err: err.message,
			});
		} catch {}
