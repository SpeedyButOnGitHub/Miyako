...
const { progressBar: sharedProgressBar, applyStandardFooter, paginationRow } = require('../ui');
...
function collectUserPermissions(member, mode = 'text') {
	...
	for (const [level, roleIdsOrArray] of Object.entries(rewards)) {
		const roleIds = Array.isArray(roleIdsOrArray)
			? roleIdsOrArray
			: roleIdsOrArray
				? [roleIdsOrArray]
				: [];
		for (const roleId of roleIds) {
			if (member.roles.cache.has(roleId)) {
				const label =
					PERMISSION_ROLE_LABELS[roleId] || (member.guild.roles.cache.get(roleId)?.name ?? roleId);
...
function buildRows(view = 'main', page = 1, totalPages = 1, mode = 'text') {
	...
	const navRow = new ActionRowBuilder().addComponents(
		new ButtonBuilder()
			.setCustomId('prof_home')
			.setLabel('👤 Profile')
			.setStyle(isProfile ? ButtonStyle.Primary : ButtonStyle.Secondary),
		new ButtonBuilder()
			.setCustomId('prof_rank')
			.setLabel('📊 Rank')
			.setStyle(isRank ? ButtonStyle.Primary : ButtonStyle.Secondary),
		new ButtonBuilder()
			.setCustomId('prof_lb')
			.setLabel('🏆 Leaderboard')
			.setStyle(isLB ? ButtonStyle.Primary : ButtonStyle.Secondary),
	);
...
async function handleProfileCommand(client, message) {
	...
	const modifier =
		getUserModifier(userId) *
		(Number.isFinite(config.globalXPMultiplier) ? config.globalXPMultiplier : 1);
	...
	if (upcoming) {
		...
		if (roleIds.length) {
			...
			embed.addFields({
				name: `${theme.emojis.action} Next Unlock`,
				value: `Level ${upcoming}: ${mentions}`,
				inline: false,
			});
		}
	}
...
ActiveMenus.registerHandler('profile', async (interaction, session) => {
	...
	if (interaction.customId === 'prof_toggle_mode') {
		...
		if (session.data.view === 'leaderboard') {
			...
			await interaction.update({
				embeds: [embed],
				components: buildRows('leaderboard', Math.min(page, totalPages), totalPages, m),
			});
			return;
			...
			const globalMod =
				typeof config.globalXPMultiplier === 'number' ? config.globalXPMultiplier : 1.0;
			...
			const pEmbed = createEmbed({
				title:
					m === 'text'
						? `${theme.emojis.profile} Your Profile`
						: `${theme.emojis.vc} Your VC Profile`,
			...
			safeAddField(
				pEmbed,
				m === 'text' ? 'XP Modifier' : 'VC XP Modifier',
				`x${eff.toFixed(2)}`,
				true,
			);
			...
			safeAddField(
				pEmbed,
				'Bank',
				`$${bankNow.toLocaleString()}/$${baseNow.toLocaleString()}`,
				true,
			);
			...
			safeAddField(
				pEmbed,
				'Unlocked Perks',
				formatPermissionPhrases(collectUserPermissions(member, m)),
			);
			...
			const nextTier = Object.keys(rewardsMapToggle)
				.map((n) => Number(n))
				.filter((n) => Number.isFinite(n) && n > userLvl)
				.sort((a, b) => a - b)[0];
			if (nextTier) {
	...
	if (interaction.customId === 'prof_lb') {
		...
		await interaction.update({
			embeds: [lbEmbed],
			components: buildRows('leaderboard', page, totalPages, mode),
		});
		return;
	}
...
async function handleRankCommand(client, message) {
	...
	const sent = await message
		.reply({ embeds: [embed], components: buildRows('rank', 1, 1, mode) })
		.catch(() => null);
	if (sent) {
		ActiveMenus.registerMessage(sent, {
			type: 'profile',
