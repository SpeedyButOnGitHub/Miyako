...
const configCategories = {
	...
	Leveling: {
		...
		settings: {
			...
			RoleXPBlacklist: {
				...
				getDisplay: () =>
					config.roleXPBlacklist && config.roleXPBlacklist.length
			...
			GlobalXPMultiplier: {
				description: () =>
					`Set a global XP multiplier applied to all XP gains. Current: **x${(config.globalXPMultiplier ?? 1).toFixed(2)}**`,
				getDisplay: () => {
					const mult =
						typeof config.globalXPMultiplier === 'number' ? config.globalXPMultiplier : 1;
			...
			LevelRewards: {
				description:
					'Configure roles automatically granted at levels. Supports multiple roles per level.',
				getDisplay: () => {
			...
			VCLevelRewards: {
				description:
					'Configure roles granted at VC levels (voice leveling). Separate from text chat rewards.',
				getDisplay: () => {
	...
	Autoroles: {
		description:
			'Automatically-assign roles to new members. Manage member autoroles and the bot-only autorole.',
		settings: {
			AutoRoles: {
				description:
					'Roles that will be automatically assigned to new members. Order matters; you can remove by number.',
				getDisplay: () => {
			...
			BotRole: {
				...
				buttons: [{ id: 'botRole', label: 'Edit', style: ButtonStyle.Primary, emoji: '�' }],
			},
		},
	},
};
