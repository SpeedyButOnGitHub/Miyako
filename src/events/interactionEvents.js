...
const {
	...
	CHATBOX_BUTTON_ID,
	isModerator,
} = require('../commands/moderation/permissions');
...
const { handleClockInSelect: _handleClockInSelect } = require('../utils/clockinHandler');

// Expose the shared handler for tests and for other modules to call
async function handleClockInSelect(interaction) {
	return _handleClockInSelect(interaction);
}
...
function attachInteractionEvents(client) {
	...
	client.on('interactionCreate', async (interaction) => {
		...
		try {
			require('../utils/commandLogger').instrumentInteractionLogging(interaction);
		} catch {}
		...
		const __clearInteractionSafety = () => {
			try {
				if (__interactionSafetyTimer) {
					clearTimeout(__interactionSafetyTimer);
					__interactionSafetyTimer = null;
				}
			} catch {}
		};
		try {
			...
			if (_isComponent) {
				...
				const wrap = (name) => {
					try {
						...
						if (fn && fn.mock && typeof fn.mockImplementation === 'function') {
							...
							try {
								if (typeof fn.getMockImplementation === 'function')
									existingImpl = fn.getMockImplementation();
							} catch (e) {}
							try {
								if (!existingImpl && typeof fn._getMockImplementation === 'function')
							...
							if (existingImpl) {
								fn.mockImplementation(async (...args) => {
									__clearInteractionSafety();
									return existingImpl.apply(interaction, args);
								});
							}
							return;
						}
				...
				__interactionSafetyTimer = setTimeout(async () => {
					try {
						...
						if (
							...
							try {
								require('../utils/logger').info('[Interaction Safety Defer]', {
				...
				if (__interactionSafetyTimer && typeof __interactionSafetyTimer.unref === 'function')
					__interactionSafetyTimer.unref();
			}
		} catch (e) {
			__clearInteractionSafety();
		}
		try {
			...
			if (
				...
				interaction.customId.startsWith('cash:check')
			) {
				...
				try {
					...
					const dropActive =
						activeDrops &&
						Array.from(activeDrops.values()).some(
							(d) => d && !d.claimedBy && d.expiresAt > Date.now(),
						);
					if (dropActive) {
						const BROADCAST_CHANNEL_ID = '1232701768987578462';
						const ch = await interaction.client.channels
							.fetch(BROADCAST_CHANNEL_ID)
							.catch(() => null);
						if (ch && ch.send) {
							const pubMsg = await ch
								.send({
									content: `🔍 Balance Check: <@${interaction.user.id}>`,
									...payload,
									allowedMentions: { users: [interaction.user.id] },
								})
								.catch(() => null);
							if (pubMsg)
								jumpLink = `https://discord.com/channels/${pubMsg.guildId}/${pubMsg.channelId}/${pubMsg.id}`;
						}
					}
				} catch {}
				if (jumpLink) {
					await interaction
						.reply({ content: `Balance posted here → ${jumpLink}`, flags: 1 << 6 })
						.catch(() => {});
				} else {
			...
			if (interaction.isButton() && interaction.customId.startsWith('apply_app_')) {
				...
				try {
					...
					if (!Array.isArray(app.questions) || app.questions.length === 0)
						return interaction
							.reply({ content: 'This application has no questions configured.', flags: 1 << 6 })
							.catch(() => {});
					const member = interaction.member;
					if (!member)
						return interaction
							.reply({ content: 'Member not resolved.', flags: 1 << 6 })
							.catch(() => {});
					const gate = canApply(member, app);
					if (!gate.ok)
						return interaction.reply({ content: gate.reason, flags: 1 << 6 }).catch(() => {});
					// Rate limit: if a pending submission for this app exists in last 24h for this user
					const recent = listSubmissions({}).filter(
						(s) =>
							s.appId === String(app.id) &&
							s.userId === member.id &&
							s.status === 'pending' &&
					...
					if (recent.length) {
						return interaction
							.reply({
								content:
									'You already have a pending application for this in the last 24h. Please wait for a decision.',
								flags: 1 << 6,
							})
							.catch(() => {});
					}
					...
					if (sess.index >= app.questions.length) {
						const confirmRow = new ActionRowBuilder().addComponents(
							new ButtonBuilder()
								.setCustomId(`appconfirm_submit_${app.id}`)
								.setLabel('Submit')
								.setStyle(ButtonStyle.Success),
							new ButtonBuilder()
								.setCustomId(`appconfirm_cancel_${app.id}`)
								.setLabel('Cancel')
								.setStyle(ButtonStyle.Danger),
						);
						return interaction
							.reply({
								content: app.confirmMessage || 'Submit application?',
					...
					const modal = new ModalBuilder()
						.setCustomId(modalId)
						.setTitle(`Q${sess.index + 1}/${app.questions.length}`);
					...
					const submitted = await interaction
						.awaitModalSubmit({
							time: 5 * 60 * 1000,
							filter: (i) => i.customId === modalId && i.user.id === member.id,
						})
						.catch(() => null);
					if (!submitted) {
						return;
					...
					if (sess.index >= app.questions.length) {
						...
						const embed = new EmbedBuilder()
							.setTitle(`${app.name} — Preview`)
							.setDescription('Review your answers then submit.')
							.setColor(0x5865f2);
						for (const qa of sess.answers.slice(0, 15)) {
							const qq = app.questions.find((x) => String(x.id) === String(qa.qid));
							if (!qq) continue;
							embed.addFields({
								name: qq.label.slice(0, 256),
								value: qa.answer.slice(0, 1024) || '*blank*',
							});
						}
						...
						await submitted
							.reply({ embeds: [embed], components: [row], flags: 1 << 6 })
							.catch(() => {});
					} else {
						await submitted
							.reply({
								content: `Saved answer (${sess.index}/${app.questions.length}). Use the Apply button again for next question.`,
								flags: 1 << 6,
							})
							.catch(() => {});
					}
					...
					try {
						require('../utils/logger').error('[apply_app_flow] error', { err: e.message });
					} catch {}
					return interaction
						.reply({ content: 'Error starting flow.', flags: 1 << 6 })
						.catch(() => {});
				}
			}
			...
			if (
				interaction.isButton() &&
				(interaction.customId.startsWith('appconfirm_submit_') ||
					interaction.customId.startsWith('appconfirm_cancel_'))
			) {
				...
				if (!sess)
					return interaction
						.reply({ content: 'Session expired. Start again.', flags: 1 << 6 })
						.catch(() => {});
				if (action === 'cancel') {
					abandonSession(interaction.user.id, appId);
					return interaction
						.reply({ content: 'Application cancelled.', flags: 1 << 6 })
						.catch(() => {});
				}
					...
					try {
						...
						if (ch) {
							...
							const emb = new EmbedBuilder()
								.setTitle(`${app.name} Submission #${submission.id}`)
							...
							const row = new ActionRowBuilder().addComponents(
								new ButtonBuilder()
									.setCustomId(`appreview_accept_${app.id}_${submission.id}`)
									.setLabel('Accept')
									.setStyle(ButtonStyle.Success),
								new ButtonBuilder()
									.setCustomId(`appreview_deny_${app.id}_${submission.id}`)
									.setLabel('Deny')
									.setStyle(ButtonStyle.Danger),
							);
				...
				return interaction
					.reply({ content: app.completionMessage || 'Application submitted.', flags: 1 << 6 })
					.catch(() => {});
			}
			...
			if (interaction.isButton() && interaction.customId.startsWith('appreview_')) {
				...
				try {
					...
					if (!current)
						return interaction
							.reply({ content: 'Submission missing.', flags: 1 << 6 })
							.catch(() => {});
					if (current.status !== 'pending') {
						return interaction
							.reply({ content: `Already ${current.status}.`, flags: 1 << 6 })
							.catch(() => {});
					}
					...
					if (!updated)
						return interaction.reply({ content: 'Update failed.', flags: 1 << 6 }).catch(() => {});
						...
						if (guildMember) {
							if (newStatus === 'accepted') {
								...
								for (const r of app.acceptedRoles || []) {
									if (!guildMember.roles.cache.has(r))
										await guildMember.roles.add(r).catch(() => {});
								}
					...
					try {
						await interaction.update({
							content: `Submission ${newStatus.toUpperCase()}.`,
					...
					try {
						require('../utils/logger').error('[appreview] error', { err: e.message });
					} catch {}
			...
			if (interaction.isButton() && interaction.customId === 'bank:menu:deposit') {
				await interaction.deferUpdate().catch(() => {});
				try {
					await interaction.message.edit(buildDepositMenuPayload(interaction.user.id));
				} catch {}
				return;
			}
			if (interaction.isButton() && interaction.customId === 'bank:menu:withdraw') {
				await interaction.deferUpdate().catch(() => {});
				try {
					await interaction.message.edit(buildWithdrawMenuPayload(interaction.user.id));
				} catch {}
				return;
			}
			if (interaction.isButton() && interaction.customId === 'bank:back') {
				await interaction.deferUpdate().catch(() => {});
				try {
					await interaction.message.edit(buildBalancePayload(interaction.user.id));
				} catch {}
				return;
			}
			...
			if (interaction.isButton() && interaction.customId === 'bank:deposit:amount') {
				...
				if (amt <= 0) {
					await submitted.reply({ content: '❌ Enter a positive amount.', flags: 1 << 6 });
					return;
				}
				const q = quoteDeposit(interaction.user.id, amt);
				if (!q.ok) {
					await submitted.reply({ content: '❌ Invalid amount.', flags: 1 << 6 });
					return;
				}
				...
				const row = new ActionRowBuilder().addComponents(
					semanticButton('success', {
						id: `bank:confirm:${q.deposit}:${q.tax}:${q.totalCost}:${rootMsgId}`,
						label: 'Confirm',
					}),
			...
			if (interaction.isButton() && interaction.customId === 'bank:deposit:max') {
				...
				if (maxAff.deposit <= 0) {
					await interaction
						.reply({ content: 'Nothing to deposit.', flags: 1 << 6 })
						.catch(() => {});
					return;
				}
				...
				const warn = warnNeeded
					? `\n\n⚠️ **Warning:** Above / crossing daily limit. Tax: **$${maxAff.tax.toLocaleString()}** (${taxPct.toFixed(1)}%) (Total Cost: **$${maxAff.totalCost.toLocaleString()}**).`
					: maxAff.tax
				...
				const row = new ActionRowBuilder().addComponents(
					semanticButton('danger', {
						id: `bank:confirm:${maxAff.deposit}:${maxAff.tax}:${maxAff.totalCost}:${rootMsgId}`,
						label: 'Confirm Max',
					}),
					semanticButton('primary', { id: `bank:confirm:toLimit:${rootMsgId}`, label: 'To Limit' }),
					semanticButton('nav', { id: 'bank:decline', label: 'Cancel' }),
			...
			if (interaction.isButton() && interaction.customId === 'bank:withdraw:amount') {
				...
				if (amt > getBank(interaction.user.id)) {
					await submitted.reply({
						content: "❌ You don't have that much in the bank.",
						flags: 1 << 6,
					});
					return;
				}
				...
				const row = new ActionRowBuilder().addComponents(
					semanticButton('success', {
						id: `bank:withdraw:confirm:${amt}:${rootMsgId}`,
						label: 'Confirm',
					}),
					semanticButton('nav', { id: 'bank:decline', label: 'Cancel' }),
				);
			...
			if (interaction.isButton() && interaction.customId === 'bank:withdraw:max') {
				...
				if (bankBal <= 0) {
					await interaction.reply({ content: 'Bank is empty.', flags: 1 << 6 }).catch(() => {});
					return;
				}
				const rootMsgId = interaction.message?.id;
				const row = new ActionRowBuilder().addComponents(
					semanticButton('danger', {
						id: `bank:withdraw:confirm:${bankBal}:${rootMsgId}`,
						label: 'Confirm Withdraw All',
					}),
					semanticButton('nav', { id: 'bank:decline', label: 'Cancel' }),
				);
				await interaction
					.reply({
						content: `Withdraw all $${bankBal.toLocaleString()}?`,
						components: [row],
						flags: 1 << 6,
					})
					.catch(() => {});
			...
			if (interaction.isButton() && interaction.customId.startsWith('bank:confirm:toLimit')) {
				...
				if (bankBal >= base) {
					await interaction
						.reply({ content: 'Already at or above the daily limit.', flags: 1 << 6 })
						.catch(() => {});
					return;
				}
				...
				if (!q.ok) {
					await interaction
						.reply({ content: '❌ Could not compute required amount.', flags: 1 << 6 })
						.catch(() => {});
					return;
				}
				...
				if (!res.ok) {
					await interaction
						.reply({ content: `❌ ${res.error || 'Deposit failed'}`, flags: 1 << 6 })
						.catch(() => {});
					return;
				}
				...
				await interaction
					.reply({
						content: `✅ Deposited $${res.moved.toLocaleString()} to reach the daily limit.${res.tax ? ` Tax $${res.tax.toLocaleString()} (${pct.toFixed(1)}%)` : ''}`,
						flags: 1 << 6,
					})
					.catch(() => {});
				...
				if (rootId) {
					try {
						...
						if (channel?.messages?.fetch) {
							...
							if (rootMsg)
								await rootMsg.edit(buildBalancePayload(interaction.user.id)).catch(() => {});
						}
					} catch {}
				}
				return;
			...
			if (interaction.isButton() && interaction.customId.startsWith('bank:confirm:')) {
				...
				await interaction
					.reply({
						content: `✅ Deposited $${res.moved.toLocaleString()}${tax ? ` (Tax $${tax.toLocaleString()})` : ''}.`,
						flags: 1 << 6,
					})
					.catch(() => {});
			...
			if (interaction.isButton() && interaction.customId.startsWith('bank:withdraw:confirm:')) {
				...
				if (!res.ok) {
					await interaction
						.reply({ content: `❌ ${res.error || 'Withdraw failed'}`, flags: 1 << 6 })
						.catch(() => {});
					return;
				}
				await interaction
					.reply({ content: `✅ Withdrew $${res.moved.toLocaleString()}.`, flags: 1 << 6 })
					.catch(() => {});
			...
			if (
				...
				(interaction.type === InteractionType.ModalSubmit &&
					interaction.customId?.startsWith('warns:'))
			) {
			...
			if (interaction.isButton() && interaction.customId?.startsWith('modact:')) {
				...
				const kind = parts[1];
				const act =
					kind === 'menu' || ['init', 'confirm', 'cancel', 'changeReason', 'back'].includes(kind)
						? null
						: kind;
				const group = kind === 'menu' ? parts[2] : null;
				const flowAction = ['init', 'confirm', 'cancel', 'changeReason'].includes(kind)
					? parts[2]
					: null; // kick|ban
				const uid =
					kind === 'menu'
						? parts[3]
						: ['init', 'confirm', 'cancel', 'changeReason', 'back'].includes(kind)
							? parts[3]
							: parts[2];
				const durMs = parts[4]
					? Number(parts[4])
					: parts[3] &&
						  !['menu', 'init', 'confirm', 'cancel', 'changeReason', 'back'].includes(kind)
						? Number(parts[3])
						: null;
				...
				if (!member || !isModerator(member)) {
					await interaction
						.reply({ content: `${EMOJI_ERROR} You are not allowed to use this.`, flags: 1 << 6 })
						.catch(() => {});
					return;
				}
				...
				if (!targetMember && !targetUser) {
					await interaction
						.reply({ content: `${EMOJI_ERROR} Could not resolve user.`, flags: 1 << 6 })
						.catch(() => {});
					return;
				}
					...
					if (targetInGuild) {
						...
						if (!actorOwner) {
							...
							if (actorHighest <= targetHighest) {
								return {
									ok: false,
									msg: `${EMOJI_ERROR} You cannot act on a member with an equal or higher role.`,
								};
							}
						}
						...
						if (botHighest <= targetHighest) {
							return {
								ok: false,
								msg: `${EMOJI_ERROR} I cannot act on a member with an equal or higher role than mine.`,
							};
						}
					}
					return { ok: true };
				...
				const ensureStores = () => {
					...
					if (typeof config.testingWarnings !== 'object' || !config.testingWarnings)
				...
				if (kind === 'menu') {
					if (group === 'warnings') {
						...
						const rows = [
							new ActionRowBuilder().addComponents(
								semanticButton('danger', {
									id: `modact:mute:${uid}:3600000`,
									label: '1h',
									emoji: '⏱️',
								}),
								semanticButton('danger', {
									id: `modact:mute:${uid}:7200000`,
									label: '2h',
									emoji: '⏱️',
								}),
								semanticButton('danger', {
									id: `modact:mute:${uid}:21600000`,
									label: '6h',
									emoji: '⏱️',
								}),
								semanticButton('danger', {
									id: `modact:mute:${uid}:86400000`,
									label: '24h',
									emoji: '⏱️',
								}),
							),
				...
				if (
					(kind === 'confirm' || kind === 'cancel' || kind === 'changeReason') &&
					(flowAction === 'kick' || flowAction === 'ban')
				) {
					const key = `${uid}:${flowAction}:${interaction.user.id}`;
					...
					if (kind === 'cancel') {
						await interaction
							.reply({
								content: `${EMOJI_SUCCESS} ${flowAction === 'kick' ? 'Kick' : 'Ban'} cancelled.`,
								flags: 1 << 6,
							})
							.catch(() => {});
						...
						try {
							if (state.originChannelId && state.originMessageId) {
								const channel = await client.channels
									.fetch(state.originChannelId)
									.catch(() => null);
								const msg = await channel?.messages
									?.fetch?.(state.originMessageId)
									.catch(() => null);
								if (msg) {
					...
					if (kind === 'changeReason') {
						...
						if (submitted) {
							...
							pendingPunishments.set(key, {
								reason: newReason,
								originChannelId: state.originChannelId,
							...
							const row = new ActionRowBuilder().addComponents(
								...
								semanticButton('danger', {
									id: `modact:cancel:${flowAction}:${uid}`,
									label: 'Cancel',
								}),
								semanticButton('primary', {
									id: `modact:changeReason:${flowAction}:${uid}`,
					...
					if (kind === 'confirm') {
									...
									if (msg) {
										...
										const buildTopRow = () =>
											new ActionRowBuilder().addComponents(
												...
												semanticButton('nav', {
													id: `modact:menu:mute:${uid}`,
													label: 'Mute',
													emoji: '⏰',
												}),
												semanticButton('nav', {
													id: `modact:init:kick:${uid}`,
													label: 'Kick',
													emoji: '👢',
												}),
												semanticButton('danger', {
													id: `modact:init:ban:${uid}`,
													label: 'Ban',
													emoji: '🔨',
												}),
											);
						...
						await interaction
							.reply({
								content: `${EMOJI_SUCCESS} ${flowAction === 'kick' ? 'Kick' : 'Ban'} issued${isTesting ? ' (testing)' : ''}.`,
								flags: 1 << 6,
							})
							.catch(() => {});
						// restore original message buttons to top row
				...
				if (act === 'addwarn') {
					...
					if (total >= kickT) {
						...
						if (!isTesting && targetMember) {
							try {
								await targetMember.timeout(Math.min(muteMs, 14 * 24 * 60 * 60 * 1000));
							} catch {}
						}
						const ms = require('ms');
						durText = ms(muteMs, { long: true });
					}
					// remaining line
					let remainingLine = null;
					if (total < muteT)
						remainingLine = `${muteT - total} warning${muteT - total === 1 ? '' : 's'} remaining until mute`;
					else if (total < kickT)
						remainingLine = `${kickT - total} warning${kickT - total === 1 ? '' : 's'} remaining until kick`;
					...
					const nxtRemain = remainingLine
						? parseInt((remainingLine.match(/^(\d+)/) || [0, 0])[1], 10) || 0
						: 0;
					await sendModLog(
						client,
						targetMember || targetUser,
				...
				if (act === 'removewarn') {
					...
					let remainingLine = null;
					if (arr.length < muteT)
						remainingLine = `${muteT - arr.length} warning${muteT - arr.length === 1 ? '' : 's'} remaining until mute`;
					else if (arr.length < kickT)
						remainingLine = `${kickT - arr.length} warning${kickT - arr.length === 1 ? '' : 's'} remaining until kick`;
					const nxtRemain = remainingLine
						? parseInt((remainingLine.match(/^(\d+)/) || [0, 0])[1], 10) || 0
						: 0;
					await sendUserDM(targetMember || targetUser, 'warning removed', null, null, null);
					const reasonForLog = remainingLine
						? `${removed?.reason || 'No reason'}\n\n${remainingLine}`
						: `${removed?.reason || 'No reason'}`;
					await sendModLog(
						client,
						targetMember || targetUser,
				...
				if (act === 'mute') {
					...
					if (
						!isTesting &&
						targetMember &&
						typeof targetMember.timeout === 'function' &&
						Number.isFinite(durMs)
					) {
						try {
							await targetMember.timeout(Math.min(durMs, 14 * 24 * 60 * 60 * 1000));
						} catch {}
					}
					const ms = require('ms');
					...
					const rows = [
						new ActionRowBuilder().addComponents(
							new ButtonBuilder()
								.setCustomId(`modact:mute:${uid}:3600000`)
								.setLabel('1h')
								.setStyle(ButtonStyle.Danger)
								.setEmoji('⏱️'),
							new ButtonBuilder()
								.setCustomId(`modact:mute:${uid}:7200000`)
								.setLabel('2h')
								.setStyle(ButtonStyle.Danger)
								.setEmoji('⏱️'),
							new ButtonBuilder()
								.setCustomId(`modact:mute:${uid}:21600000`)
								.setLabel('6h')
								.setStyle(ButtonStyle.Danger)
								.setEmoji('⏱️'),
							new ButtonBuilder()
								.setCustomId(`modact:mute:${uid}:86400000`)
								.setLabel('24h')
								.setStyle(ButtonStyle.Danger)
								.setEmoji('⏱️'),
						),
						new ActionRowBuilder().addComponents(
							new ButtonBuilder()
								.setCustomId(`modact:unmute:${uid}`)
								.setLabel('Unmute')
								.setStyle(ButtonStyle.Success)
								.setEmoji('✅')
								.setDisabled(!isTimedOut),
							new ButtonBuilder()
								.setCustomId(`modact:back:${uid}`)
								.setLabel('Back')
				...
				if (act === 'unmute') {
					if (!isTesting && targetMember) {
						try {
							if (typeof targetMember.timeout === 'function') await targetMember.timeout(null);
						} catch {}
					}
				...
				await interaction
					.reply({ content: `${EMOJI_ERROR} Unknown action.`, flags: 1 << 6 })
					.catch(() => {});
				return;
			}
			...
			if (
				interaction.type === InteractionType.ModalSubmit &&
				interaction.customId === 'staffteam_chatbox_modal'
			) {
				const member = interaction.member;
				const hasRole = member.roles.cache.some((role) => ALLOWED_ROLES.includes(role.id));
				if (!hasRole) {
			...
			if (interaction.isButton() && interaction.customId.startsWith('event_notify_')) {
				...
				if (!member) {
					await interaction
						.reply({ content: 'Could not resolve member.', flags: 1 << 6 })
						.catch(() => {});
					return;
				}
				...
				try {
					if (hasRole) {
						...
						await interaction
							.reply({
								content: `Removed role: <@&${NOTIFY_ROLE_ID}>. You will no longer receive notifications for this event.`,
								flags: 1 << 6,
							})
							.catch(() => {});
					} else {
						...
						await interaction
							.reply({
								content: `Granted role: <@&${NOTIFY_ROLE_ID}>. You will now be notified whenever this event starts.`,
								flags: 1 << 6,
							})
							.catch(() => {});
					}
					...
					await interaction
						.reply({ content: 'Failed to toggle role: ' + (e.message || e), flags: 1 << 6 })
						.catch(() => {});
				}
				return;
			}
			...
			if (
				...
				if (!channel) {
					await interaction.reply({
						content: `${EMOJI_ERROR} Invalid or unknown channel.`,
						flags: 1 << 6,
					});
					return;
				}
				...
				if (mode === 'whitelist') {
					if (action === 'addChannel') {
						...
						if (!config.snipingWhitelist.includes(channel.id)) {
							...
							saveConfig();
							await interaction.reply({
								content: `${EMOJI_SUCCESS} Added <#${channel.id}> to whitelist.`,
								flags: 1 << 6,
							});
						} else {
							await interaction.reply({
								content: `${EMOJI_ERROR} Channel already in whitelist.`,
								flags: 1 << 6,
							});
						}
					} else if (action === 'removeChannel') {
						if (
							Array.isArray(config.snipingWhitelist) &&
							config.snipingWhitelist.includes(channel.id)
						) {
							config.snipingWhitelist = config.snipingWhitelist.filter((id) => id !== channel.id);
							saveConfig();
							await interaction.reply({
								content: `${EMOJI_SUCCESS} Removed <#${channel.id}> from whitelist.`,
								flags: 1 << 6,
							});
						} else {
							await interaction.reply({
								content: `${EMOJI_ERROR} Channel not in whitelist.`,
								flags: 1 << 6,
							});
						}
					} else {
					...
					if (action === 'addChannel') {
						...
						if (!config.snipingChannelList.includes(channel.id)) {
							...
							saveConfig();
							await interaction.reply({
								content: `${EMOJI_SUCCESS} Added <#${channel.id}> to blacklist.`,
								flags: 1 << 6,
							});
						} else {
							await interaction.reply({
								content: `${EMOJI_ERROR} Channel already in blacklist.`,
								flags: 1 << 6,
							});
						}
					} else if (action === 'removeChannel') {
						if (
							Array.isArray(config.snipingChannelList) &&
							config.snipingChannelList.includes(channel.id)
						) {
							config.snipingChannelList = config.snipingChannelList.filter(
								(id) => id !== channel.id,
							);
							saveConfig();
							await interaction.reply({
								content: `${EMOJI_SUCCESS} Removed <#${channel.id}> from blacklist.`,
								flags: 1 << 6,
							});
						} else {
							await interaction.reply({
								content: `${EMOJI_ERROR} Channel not in blacklist.`,
								flags: 1 << 6,
							});
						}
					} else {
			...
			if (
				interaction.type === InteractionType.ModalSubmit &&
				interaction.customId.startsWith('event_create_modal')
			) {
			...
			if (
				interaction.type === InteractionType.ModalSubmit &&
				/^event_(times|days|msg|edit)_modal_/.test(interaction.customId)
			) {
			...
			if (
				interaction.type === InteractionType.ModalSubmit &&
				/^(notif_(add|offset|msg|channel|edit)_modal_)/.test(interaction.customId)
			) {
			...
			if (interaction.isButton() && interaction.customId.startsWith('clockin:autoNext:')) {
				...
				if (roleKey === 'none') {
					await interaction
						.reply({ content: 'Select a role first.', flags: 1 << 6 })
						.catch(() => {});
					return;
				}
				const clock =
				...
				try {
					...
					const msgTargets = [];
					if (Array.isArray(clock.messageIds))
						for (const id of clock.messageIds)
							msgTargets.push({
								id,
								channelId: clock.channelId || ev.channelId || interaction.channelId,
							});
					...
					for (const t of msgTargets) {
						...
						try {
							const ch = t.channelId
								...
								: interaction.channel;
							const msg =
								ch && ch.messages ? await ch.messages.fetch(t.id).catch(() => null) : null;
				...
				try {
					...
					await interaction
						.reply({
							content: `You will be auto-registered as ${roleKey.replace(/_/g, ' ')} next clock-in.`,
							components: [row],
							flags: 1 << 6,
						})
						.catch(() => {});
			...
			if (interaction.isButton() && interaction.customId.startsWith('clockin:autoNextCancel:')) {
				...
				if (clock.autoNext && clock.autoNext[interaction.user.id]) {
					...
					await interaction
						.reply({ content: 'Auto registration cancelled.', flags: 1 << 6 })
						.catch(() => {});
				} else {
					await interaction
						.reply({ content: 'No auto registration found.', flags: 1 << 6 })
						.catch(() => {});
				}
				return;
			}
			...
			try {
				...
				try {
					require('../utils/logger').error('[Interaction Error] (logging failed)', {
						err: e.message,
					});
				} catch {}
			}
			...
			try {
				...
				try {
					require('../utils/logger').error('[Interaction Error] (post to channel failed)', {
						err: e.message,
					});
				} catch {}
			}
			...
			try {
				...
				if (isComponent && canReply && !interaction.replied && !interaction.deferred) {
					try {
						await interaction.deferUpdate().catch(() => {});
						try {
							require('../utils/logger').info('[Interaction Fallback Ack]', {
								customId: interaction?.customId || null,
								userId: interaction?.user?.id || null,
							});
						} catch {}
						...
						try {
							...
							const shouldPost = !!(
								cfg &&
								(cfg.postInteractionErrorsToLogChannel || cfg.debugMode)
							);
							if (shouldPost) {
								const { CONFIG_LOG_CHANNEL } = require('../utils/logChannels');
								if (CONFIG_LOG_CHANNEL && interaction?.client?.channels?.fetch) {
							...
							try {
								require('../utils/logger').error(
									'[Interaction Fallback Ack] (post to channel failed)',
									{ err: e.message },
								);
							} catch {}
						}
				...
				try {
					require('../utils/logger').error('[Interaction Fallback Ack Failure]', {
						err: e.message,
					});
				} catch {}
			}
