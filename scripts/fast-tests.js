...
const localJest = path.join(root, 'node_modules', '.bin', 'jest' + (process.platform === 'win32' ? '.cmd' : ''));
...
if (!res || res.error || (typeof res.status === 'number' && res.status !== 0)) {
  ...
  try {
    ...
    res = spawnSync('npx jest ' + args.map(a => (a.includes(' ') ? `"${a}"` : a)).join(' '), { stdio: 'inherit', cwd: root, shell: true });
  } catch (err) {
