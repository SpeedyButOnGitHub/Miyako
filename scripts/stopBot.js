		...
		if (process.platform === 'win32') {
			...
			const out = spawnSync(
				...
				[
					'-NoProfile',
					'-Command',
					"Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -and $_.CommandLine -like '*index.js*' } | Select-Object ProcessId,CommandLine | ConvertTo-Json",
				],
				{ encoding: 'utf8' },
			);
...
if (!fs.existsSync(pidFile)) {
	...
	console.log(
		'Found candidate bot processes:',
		candidates.map((p) => p.pid),
	);
	...
	setTimeout(() => {
		for (const p of candidates) {
			if (processExists(p.pid)) {
				if (process.platform === 'win32') {
					try {
						require('child_process').spawnSync('taskkill', ['/PID', String(p.pid), '/T', '/F']);
					} catch (e) {
						/* ignore */
					}
				} else {
		...
		if (!anyRunning) {
			console.log(
				'Stopped detected bot processes:',
				candidates.map((p) => p.pid),
			);
			process.exit(0);
		}
		console.error(
			'Failed to terminate detected bot processes:',
			candidates.map((p) => p.pid),
		);
		process.exit(1);
	}, 800);
