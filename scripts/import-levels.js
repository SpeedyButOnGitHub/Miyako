		...
		try {
			...
			async function ensureMemberIndex() {
				...
				try {
					...
					console.warn(
						'[import-levels] Warning: Failed to fetch all members. Name resolution may be incomplete.',
						e?.message || e,
					);
				}
				memberRecords = [];
			...
			function resolveNameToId(name) {
				...
				const nraw = norm(raw);
				if (!nraw) return null;
				const exact = memberRecords.filter(
					(m) =>
						m.u.toLowerCase() === raw.toLowerCase() ||
						m.d.toLowerCase() === raw.toLowerCase() ||
						m.nu === nraw ||
						m.nd === nraw,
				);
			...
			for (const msg of messages) {
				...
				if (!uid && parsed.userName) {
					...
					if (uid) resolvedFromNames++;
						...
						unresolvedNames.set(parsed.userName, (unresolvedNames.get(parsed.userName) || 0) + 1);
				}
				if (!uid) continue;
			...
			console.log(
				`[import-levels] Text stats: nonEmpty=${nonEmptyText}, containsCongrats=${containsCongrats}, containsLevelWord=${containsLevelWord}`,
			);
			...
			if (unresolvedNames.size) {
				...
				const top = Array.from(unresolvedNames.entries())
					.sort((a, b) => b[1] - a[1])
					.slice(0, 10);
				console.log(
					`[import-levels] Unresolved @Name samples (top ${top.length}):`,
					top.map(([n, c]) => `${n} x${c}`).join(', '),
				);
			}
			...
			console.log(
				`[import-levels] Updated ${updated} users. Entries after update (in-memory): ${afterCount}`,
			);
			try {
