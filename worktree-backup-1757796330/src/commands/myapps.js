...
async function handleMyAppsCommand(client, message) {
  try {
    ...
    const subs = listSubmissions({}).filter(s => s.userId === userId).sort((a,b) => b.createdAt - a.createdAt).slice(0,10);
    if (!subs.length) {
      return message.reply({ content: 'You have no application submissions yet.' });
    }
    const e = createEmbed({ title: 'My Applications', description: `Showing latest ${subs.length} submission(s).` });
    const statusEmoji = (s) => s==='pending'? '⏳' : (s==='accepted' ? '✅' : (s==='denied' ? '❌' : '•'));
    for (const sub of subs) {
      const app = getApplication(sub.appId) || { name: `App ${sub.appId}` };
      const age = formatRelative(sub.createdAt);
      const decided = sub.decidedAt ? ` — decided ${formatRelative(sub.decidedAt)} by ${sub.decidedBy?`<@${sub.decidedBy}>`: 'unknown'}`: '';
      safeAddField(e, `${statusEmoji(sub.status)} ${app.name}`, `ID: #${sub.id} • ${sub.status}${decided}\nSubmitted ${age}`);
    }
    return message.reply({ embeds: [e] });
  } catch (e) {
    try { require('../utils/logger').error('[myapps] error', { err: e.message }); } catch {}
