...
describe('Applications CRUD', () => {
  ...
  test('add & fetch application', () => {
    const app = appsUtil.addApplication({ name: 'Staff App', questions: [{ id:'q1', type:'short', label:'Why?', required:true }] });
  ...
  test('submission persistence', () => {
    const app = appsUtil.addApplication({ name: 'Flow', questions: [{ id:'q1', type:'short', label:'Why?', required:true }] });
