...
const channelId = process.argv[2] && !process.argv[2].startsWith('--') ? process.argv[2] : '1232701768832516100';
...
function saveLevels(obj) {
  try { fs.writeFileSync(LEVELS_FILE, JSON.stringify(obj, null, 2)); } catch (e) { console.error('Failed saving levels.json', e); }
}
...
const FROM_TO = /(?:climbed|advanced|went)\s+from\s+(?:level|lvl|rank)\s+(\d{1,4})\s+(?:to|->|➡|➜|→)\s+(\d{1,4})/i;
...
const ANY_LEVEL_NUM = /(?:level|lvl|rank)\s+(\d{1,4})/ig;
...
async function run() {
  ...
  if (!channel || !channel.isTextBased()) {
    console.error('Channel not found or not text based:', channelId); process.exit(1);
  }
  ...
  let fetched = 0; let batches = 0; let lastId = null; const LIMIT = 100;
  let samplePrinted = 0;
  ...
  while (true) {
    ...
    for (const msg of col.values()) {
          ...
          if (nameMap.has(lower)) {
            ...
            const lev = (a,b) => {
              ...
              for (let i=1;i<=a.length;i++) for (let j=1;j<=b.length;j++) {
                const cost = a[i-1]===b[j-1]?0:1; m[i][j]=Math.min(m[i-1][j]+1,m[i][j-1]+1,m[i-1][j-1]+cost);
              }
            ...
            for (const entry of nameVariants) {
              for (const v of entry.variants) {
                ...
                if (lv === norm || lv.includes(norm) || norm.includes(lv)) { candidates.push(entry.id); break; }
                if (lev(lv, norm) <= 2) { candidates.push(entry.id); break; }
              }
            }
      ...
      if (!uid) { diagUserFailed++; continue; }
      diagUserResolved++;
      let newLevel = null;
      let m = patternText.match(FROM_TO);
      if (m) {
        const to = parseInt(m[2],10); if (Number.isFinite(to)) newLevel = to;
      } else if ((m = patternText.match(TO_ONLY))) {
        const to = parseInt(m[1],10); if (Number.isFinite(to)) newLevel = to;
        ...
        let tmp; let maxFound = 0; while ((tmp = ANY_LEVEL_NUM.exec(patternText)) !== null) { const v = parseInt(tmp[1],10); if (v > maxFound) maxFound = v; }
        if (maxFound > 0) newLevel = maxFound;
      }
      if (!newLevel) { diagPatternFailed++; continue; }
      diagPatternResolved++;
      const prev = userMaxLevels.get(uid) || 0;
      if (newLevel > prev) userMaxLevels.set(uid, newLevel);
      if (debugLimit && debugCount < debugLimit) {
        console.log(`[debug] uid=${uid} level=${newLevel} msg=${msg.id} excerpt="${fullText.slice(0,120).replace(/\n/g,' ')}"`);
        debugCount++;
      }
    }
    lastId = col.last().id;
  ...
  console.log(`Scanned ${fetched} messages in ${batches} batch(es). Found ${userMaxLevels.size} users with level records.`);
  ...
  if (userMaxLevels.size === 0) { process.exit(0); }
...
run().catch(e => { console.error('Error during reconstruction', e); process.exit(1); });
