...
describe('Applications interaction flows (mocked)', () => {
  ...
  const channel = { id: 'chan1', send: jest.fn(async ({ embeds, components }) => ({ id: 'm1', channelId: 'chan1', channel, guildId: 'g1', embeds, components })) };
  ...
  test('rename flow: showModal + awaitModalSubmit -> updates name', async () => {
    ...
    const submitted = { fields: { getTextInputValue: (k) => 'NewName' }, reply: jest.fn(async ()=>({})), update: jest.fn(async ()=>({})) };
    const interaction = {
      isButton: () => true,
      user: author,
  ...
  test('delete flow: cancel then confirm', async () => {
    ...
    const submitted = { fields: { getTextInputValue: (k) => 'DELETE' }, reply: jest.fn(async ()=>({})), update: jest.fn(async ()=>({})), user: author };
