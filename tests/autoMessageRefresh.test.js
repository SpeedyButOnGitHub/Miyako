...
test('refreshTrackedAutoMessages re-inserts mention line when forced', async () => {
  const ev = addEvent({ name: 'TestEv', channelId: 'chan-test', autoMessages: [{ id: '1', message: 'Hello', mentions: ['111'] }] });
  // create fake channel and message
  const channel = makeChannel();
  // create a live message that lacks the mention line
  const live = { id: 'live-1', content: 'Hello', embeds: [], edit: async function(p){ this.content = p.content; this.embeds = p.embeds; return this; } };
  channel._add(live);
  // set runtime notif tracking
  updateEvent(ev.id, { __notifMsgs: { '1': { channelId: channel.id, ids: ['live-1'] } } });

  const client = makeClient(channel);
  await refreshTrackedAutoMessages(client, { ...ev, __notifMsgs: { '1': { channelId: channel.id, ids: ['live-1'] } }, autoMessages: ev.autoMessages }, { forceForIds: ['1'] });
