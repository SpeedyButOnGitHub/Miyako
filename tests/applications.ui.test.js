...
const { listApplications, addApplication, removeApplication } = require('../src/utils/applications');
...
describe('Applications UI builders', () => {
  ...
  const ids = bottom.components.map(c => (c.toJSON && c.toJSON().custom_id) || c.customId || '');
  expect(ids).toEqual(expect.arrayContaining(['appmgr_apps_create', 'appmgr_apps_delete', 'appmgr_back_root']));
  ...
  expect(ids).not.toEqual(expect.arrayContaining(['appmgr_apps_prev', 'appmgr_apps_next']));
    ...
    const componentsMulti = appsModule._test.buildApplicationsListComponents(multi.page, multi.totalPages, multi.apps);
  const bottomMulti = componentsMulti[componentsMulti.length -1];
  const idsMulti = bottomMulti.components.map(c => (c.toJSON && c.toJSON().custom_id) || c.customId || '');
  // Prev should be absent on page 0, Next should be present
  expect(idsMulti).toEqual(expect.arrayContaining(['appmgr_apps_create', 'appmgr_apps_delete', 'appmgr_back_root']));
