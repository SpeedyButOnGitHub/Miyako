...
describe('deletion correlation ids', () => {
	...
	test('scheduleDeleteForMessage immediate delete logs correlationId', async () => {
		...
		const hasCorr = calls.some(
			(meta) => meta && typeof meta.correlationId === 'string' && meta.correlationId.length,
		);
	...
	test('pruneClockInMessagesOnClose logs correlationId when deleting bot messages', async () => {
		...
		const ev = {
			id: 'ev2',
			__clockIn: { messageIds: ['m-b'], channelId: ch.id },
			autoMessages: [{ isClockIn: true, deleteAfterMs: 5000, id: 'n2' }],
		};
