...
describe('Applications overflow select mapping', () => {
  test('overflow actions include expected handler ids', () => {
    ...
    const expected = ['appmgr_app_questions_', 'appmgr_app_msgs_', 'appmgr_app_deployed_', 'appmgr_app_editmsg_', 'appmgr_app_rename_', 'appmgr_app_props_', 'appmgr_app_roles_'];
  ...
  test('overflow select mappings produce valid select components', () => {
    ...
    const comps = raw.map(r => {
      ...
      if (Array.isArray(r.components)) {
        return { components: r.components.map(c => typeof c.toJSON === 'function' ? c.toJSON() : c) };
      }
      return r;
    });
    // find select menu(s)
    const selectRows = comps.filter(r => Array.isArray(r.components) && r.components.some(c => c.type === 3));
