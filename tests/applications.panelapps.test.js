...
describe('Panel apps add/remove flows', () => {
	beforeAll(() => {
		...
		const panelsPath = require('path').join(
			require('../src/utils/paths').dataDir(),
			'applicationPanels.json',
		);
		require('fs').writeFileSync(
			panelsPath,
			JSON.stringify({ nextPanelId: 1, panels: [] }, null, 2),
		);
	});
	...
	test('add and remove application from panel via modals', async () => {
		...
		await handler(
			{
				isButton: () => true,
				user: author,
				customId: `appmgr_panel_apps_${panel.id}`,
				update: jest.fn(async () => ({})),
				reply: jest.fn(async () => ({})),
				isRepliable: () => true,
				replied: false,
			},
			{ userId: author.id, data: { view: 'panelDetail', panelId: panel.id } },
		);

		// Add app via modal
		const submittedAdd = {
			fields: { getTextInputValue: (k) => app.id },
			reply: jest.fn(async () => ({})),
			user: author,
		};
		const interactionAdd = {
			isButton: () => true,
			user: author,
			customId: `panelapps_add_${panel.id}`,
		...
		await handler(interactionAdd, {
			userId: author.id,
			data: { view: 'panelApps', panelId: panel.id },
		});
		...
		const interactionRem = {
			isButton: () => true,
			user: author,
			customId: `panelapps_remove_${panel.id}`,
			showModal: jest.fn(async () => ({})),
			awaitModalSubmit: jest.fn(async () => submittedRem),
			isRepliable: () => true,
			replied: false,
		};
		await handler(interactionRem, {
			userId: author.id,
			data: { view: 'panelApps', panelId: panel.id },
		});
