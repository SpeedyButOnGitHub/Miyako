...
describe('duplication guard', () => {
  test('same message id handled once', async () => {
    const client = { on: (ev, fn) => { if (ev === 'messageCreate') client._handler = fn; }, __messageListenerAttached:false };
    ...
    const message = {
      ...
      guildId: 'g1',
      channelId: 'c1',
      reply: (payload) => { replies.push(payload); return Promise.resolve({ id:'r1', ...payload }); }
    };
