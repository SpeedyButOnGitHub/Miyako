...
describe('Schedule retry and permission edge cases', () => {
	...
	test('manualTriggerAutoMessage uses retry when deleting old messages', async () => {
		...
		const ev = scheduleService.addEvent({
			...
			autoMessages: [{ id: 'n1', message: 'CI', isClockIn: true }],
		});
		scheduleService.updateEvent(ev.id, {
			__clockIn: { messageIds: ['oldmsg'], positions: {}, autoNext: {} },
		});
		...
		const channel = {
			...
			async send(payload) {
				return { id: 'newmsg', channelId: this.id };
			},
		};
		const client = {
			channels: {
	...
	test('event_notify edit rejects bad channel id in modal', async () => {
		...
		const interaction = {
			...
			fields: {
				getTextInputValue: (k) => {
					if (k === 'channel') return '<#not-a-number>';
					if (k === 'offset') return '5m';
					if (k === 'deleteafter') return '0';
					if (k === 'message') return 'ok';
				},
			},
			reply: jest.fn(async () => ({})),
