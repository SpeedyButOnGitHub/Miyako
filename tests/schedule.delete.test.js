...
jest.mock('../src/utils/retry', () => ({
	retry: (fn) => Promise.resolve(typeof fn === 'function' ? fn() : fn),
}));
	...
	test('manualTriggerAutoMessage deletes only bot-authored previous messages', async () => {
		...
		const channel = {
			...
			messages: {
				fetch: jest
					.fn()
					.mockImplementation((mid) =>
						mid === 'old1' ? Promise.resolve(oldBotMsg) : Promise.resolve(oldUserMsg),
					),
			},
		};
		const fakeClient = {
		...
		const ev = {
			...
			channelId: 'cid',
			autoMessages: [],
			__clockIn: { messageIds: ['old1', 'old2'] },
		};
