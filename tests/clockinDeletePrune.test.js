...
describe('clock-in delete prune', () => {
	test('prunes positions to autoNext and clears consumed autoNext', () => {
		...
		const ev = addEvent({
			name: 'Prune Test',
			__clockIn: {
				positions: { manager: ['a', 'b'] },
				autoNext: { u1: 'manager', u2: { role: 'bouncer' } },
				messageIds: ['m1'],
			},
		});
		...
		expect(
			Array.isArray(updated.__clockIn.positions.manager) &&
				updated.__clockIn.positions.manager.includes('u1'),
		).toBe(true);
		expect(
			Array.isArray(updated.__clockIn.positions.bouncer) &&
				updated.__clockIn.positions.bouncer.includes('u2'),
		).toBe(true);
