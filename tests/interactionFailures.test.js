...
describe('Interaction error posting and fallback ack', () => {
  ...
  beforeEach(() => {
    ...
    const channels = { fetch: jest.fn((id) => { if (id === logChannels.CONFIG_LOG_CHANNEL) return Promise.resolve(fakeLogChannel); return Promise.resolve(null); }) };

    client = {
      on: jest.fn((evt, fn) => { client._handler = fn; }),
      channels,
  ...
  test('posts to log channel when interaction handler throws', async () => {
    ...
    jest.spyOn(eventsStorage, 'getEvent').mockImplementation(() => { throw new Error('boom-getEvent'); });
