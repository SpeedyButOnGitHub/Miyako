...
function makeChannel() {
  ...
  return {
    ...
    async send(payload) {
      ...
      const msg = { id, content: payload.content || '', embeds: payload.embeds || [], components: payload.components || [], edit: async function(p){ this.content = p.content||this.content; this.embeds = p.embeds||this.embeds; return this; }, delete: async function(){ messages.delete(this.id); return true; } };
      messages.set(id, msg);
      return msg;
    },
    _add(msg) { messages.set(msg.id, msg); },
    _has(id) { return messages.has(id); }
  };
}

function makeClient(channel) { return { channels: { async fetch(id) { if (id === channel.id) return channel; return null; } }, users: { async fetch(id) { return { id }; } } } }
...
test('member flow: selection cleared and autoNext applied on new clock-in', async () => {
  // Setup event
  const ev = addEvent({ name: 'MemFlow', channelId: 'chan-mem', autoMessages: [{ id: '10', message: 'Clockin', isClockIn: true, deleteAfterMs: 0 }] });
  // Simulate existing clock-in message and positions
  updateEvent(ev.id, { __clockIn: { messageIds: ['old-m'], positions: { manager: ['u1'] }, autoNext: {} } });
  const ch = makeChannel();
  // Add old message into the channel's map so deletion can be attempted
  ch._add({ id: 'old-m', content: 'old', embeds: [], delete: async function(){ ch._deleted = ch._deleted||[]; ch._deleted.push(this.id); messages && null; return true; } });
  const client = makeClient(ch);
  const guild = { id: 'g1', members: { cache: new Map([['u1', makeGuildMember('u1')], ['u2', makeGuildMember('u2')]]) } };
