...
describe('Applications buttons smoke test (coverage)', () => {
	...
	test('root -> apps -> create -> select -> rename -> toggle -> back flows', async () => {
		...
		const interactionSelect = {
			isButton: () => true,
			user: author,
			customId: `appmgr_app_select_${app.id}`,
			update: jest.fn(async () => ({})),
			reply: jest.fn(async () => ({})),
			isRepliable: () => true,
			replied: false,
		...
		const submitted = {
			fields: { getTextInputValue: (k) => 'BrandNewName' },
			reply: jest.fn(async () => ({})),
			update: jest.fn(async () => ({})),
			user: author,
		};
		const interactionRename = {
			isButton: () => true,
			user: author,
		...
		const interactionToggle = {
			isButton: () => true,
			user: author,
			customId: `appmgr_app_toggle_${app.id}`,
			update: jest.fn(async () => ({})),
			reply: jest.fn(async () => ({})),
			isRepliable: () => true,
			replied: false,
		...
		// Back to apps
		const interactionBack = {
			isButton: () => true,
			user: author,
			customId: 'appmgr_back_apps',
			update: jest.fn(async () => ({})),
			reply: jest.fn(async () => ({})),
			isRepliable: () => true,
			replied: false,
	...
	test('panel deploy and apply flow', async () => {
		...
		const panel = require('../src/utils/applications').addPanel({
			name: 'P1',
		...
		const interactionDeploy = {
			isButton: () => true,
			user: author,
			customId: `appmgr_panel_deploy_${panel.id}`,
			channel,
			reply: jest.fn(async () => ({})),
			isRepliable: () => true,
			replied: false,
		...
		await expect(async () => {
			await applyInteraction.reply({ content: 'Simulate apply' });
		}).not.toThrow();
	});
});
