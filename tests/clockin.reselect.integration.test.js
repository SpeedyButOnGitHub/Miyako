...
test('manualTriggerAutoMessage -> select -> re-select does not delete clock-in message unexpectedly', async () => {
	const ev = addEvent({
		name: 'ClockInReselect',
		channelId: 'chan-reselect',
		autoMessages: [
			{ id: '1', message: 'Clockin', isClockIn: true, enabled: true, deleteAfterMs: 0 },
		],
		nextAutoId: 2,
	});
	...
	expect(
		Object.values(fresh2.__clockIn.positions || {}).some(
			(arr) => Array.isArray(arr) && arr.includes('u1'),
		),
	).toBe(true);
