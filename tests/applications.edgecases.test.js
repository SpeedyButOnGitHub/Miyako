...
describe('Applications edge-case modal inputs', () => {
  ...
  test('approles: empty submission clears roles', async () => {
    ...
    const submittedEmpty = { fields: { getTextInputValue: (k) => '' }, reply: jest.fn(async ()=>({})), user: author };
    const interactionEmpty = { isButton: () => true, user: author, customId: `approles_manager_${app.id}`, showModal: jest.fn(async ()=>({})), awaitModalSubmit: jest.fn(async ()=>submittedEmpty), isRepliable: ()=>true, replied:false };
    await handler(interactionEmpty, { userId: author.id, data: { view: 'appRoles', appId: app.id } });
  ...
  test('appmsg: invalid JSON is stored as plain text', async () => {
    ...
    const submittedBadJson = { fields: { getTextInputValue: (k) => '{ not: valid json }' }, reply: jest.fn(async ()=>({})), user: author };
    const interactionMsg = { isButton: () => true, user: author, customId: `appmsg_edit_accept_${app.id}`, showModal: jest.fn(async ()=>({})), awaitModalSubmit: jest.fn(async ()=>submittedBadJson), isRepliable: ()=>true, replied:false };
  ...
  test('appq: add question with invalid required flag defaults to required=true', async () => {
    ...
    const submitted = { fields: { getTextInputValue: (k) => { if (k==='label') return 'Test?'; if (k==='type') return 'short'; if (k==='required') return 'maybe'; } }, reply: jest.fn(async ()=>({})), user: author };
    const interactionAdd = { isButton: () => true, user: author, customId: `appq_add_${app.id}`, showModal: jest.fn(async ()=>({})), awaitModalSubmit: jest.fn(async ()=>submitted), isRepliable: ()=>true, replied:false };
