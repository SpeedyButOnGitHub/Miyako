...
function makeGuild(meHasManage = false) {
  const me = { roles: { highest: { position: 10 } }, permissions: { has: (p) => meHasManage && p === 'ManageRoles' } };
...
function makeInteraction(client, guild) {
  ...
  return {
    ...
    guild,
    user: { id: 'u1' },
    reply: async (opts) => { replied.called = true; replied.content = opts.content; return true; },
    client
  };
}
...
test('event_notify shows friendly message when bot lacks ManageRoles', async () => {
  ...
  attachInteractionEvents({ on: (ev, fn) => { /* store listener */ } });
